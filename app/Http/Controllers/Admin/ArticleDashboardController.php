...
class ArticleDashboardController extends Controller
{
    public function index(Request $request)
    {
        ...
        // Views chart data (last N days)
        $viewsChart = $this->getViewsChartData($period);

        // Top articles by views
        $topArticles = Article::where('category', '!=', 'berita')
            ->published()
        ...
        // Recent activity
        $recentArticles = Article::where('category', '!=', 'berita')
            ->latest()
            ->take(5)
        ...
        // Bounce rate (views with time_spent < 10 seconds)
        $bounceRate = $this->getBounceRate($period);

        return Inertia::render('Admin/Articles/Dashboard', [
            'stats' => $stats,
