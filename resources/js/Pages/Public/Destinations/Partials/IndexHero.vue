<script setup>
/**
 * IndexHero.vue - Hero matching Home Page Design System
 * Fixed: Text sizes, card styling, booking count
 */
import { ref, onMounted, onBeforeUnmount, computed } from 'vue';
import { gsap } from 'gsap';
import { ScrollTrigger } from 'gsap/ScrollTrigger';
import { ScrollToPlugin } from 'gsap/ScrollToPlugin';
import { MapPin, Star, ShoppingBag, Compass, ChevronDown } from 'lucide-vue-next';
...
const counters = ref({ destinations: 0, rating: 0, bookings: 0 });
let ctx;

const scrollToDestinations = () => {
    gsap.to(window, { duration: 1, scrollTo: { y: '#destinations', offsetY: 80 }, ease: 'power3.inOut' });
};
...
onMounted(() => {
    ctx = gsap.context(() => {
        ...
        gsap.to(counters.value, {
            destinations: props.totalDestinations,
            rating: props.averageRating,
            bookings: props.totalBookings,
            duration: 2,
            ease: 'power2.out',
            delay: 0.8
        });
        ...
        // Parallax effect for background - moves slower (0.3x speed) like Home
        ...
        // Parallax effect for content - moves faster and fades out like Home
        ...
        // Scale down hero section slightly as you scroll like Home
...
<template>
    <section ref="heroRef" class="relative min-h-[70vh] flex items-center justify-center overflow-hidden pt-16 pb-12">
        <!-- Premium Gradient Background (sama seperti Home) -->
        ...
        <div ref="contentRef" class="relative z-10 max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="text-center max-w-3xl mx-auto">
                ...
                <div class="hero-item inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-white/10 backdrop-blur-md border border-white/20 text-white/90 mb-4 hover:bg-white/15 hover:border-white/30 hover:scale-105 transition-all duration-300">
                    ...
                    <span class="text-[10px] font-semibold tracking-wide">Jelajahi Keindahan Alam</span>
                    <Compass class="w-3 h-3 text-emerald-400" />
                </div>

                <!-- Title (text-xl sm:text-2xl md:text-3xl lg:text-4xl - sama seperti Home) -->
                ...
                <!-- Description (text-xs sm:text-sm - sama seperti Home) -->
                <p class="hero-item text-xs sm:text-sm text-white/60 max-w-xl mx-auto mb-8 leading-relaxed">
                    Jelajahi keindahan alam yang menakjubkan di Taman Nasional Lore Lindu, Sulawesi Tengah
                </p>

                <!-- Stats Cards (matching Home Hero style - WHITE background) -->
                <div class="hero-item grid grid-cols-3 gap-3 max-w-md mx-auto mb-8">
                    ...
                    <div class="group bg-white/10 backdrop-blur-md border border-white/15 rounded-xl p-3 hover:bg-emerald-500/15 hover:border-emerald-400/30 hover:-translate-y-1 hover:shadow-lg hover:shadow-emerald-500/10 transition-all duration-300 cursor-default">
                        <div class="w-8 h-8 mx-auto mb-1.5 rounded-lg bg-emerald-500/20 flex items-center justify-center group-hover:scale-110 group-hover:bg-emerald-500/30 transition-all duration-300">
                            <MapPin class="w-4 h-4 text-emerald-300" />
                        </div>
                        <p class="text-lg sm:text-xl font-black text-white group-hover:text-emerald-200 transition-colors">{{ Math.round(counters.destinations) }}</p>
                        <p class="text-[9px] text-white/60 uppercase tracking-wider group-hover:text-white/80 transition-colors">Destinasi</p>
                    </div>
                    ...
                    <!-- Booking (dari database status=paid) -->
                    <div class="group bg-white/10 backdrop-blur-md border border-white/15 rounded-xl p-3 hover:bg-cyan-500/15 hover:border-cyan-400/30 hover:-translate-y-1 hover:shadow-lg hover:shadow-cyan-500/10 transition-all duration-300 cursor-default">
                        <div class="w-8 h-8 mx-auto mb-1.5 rounded-lg bg-cyan-500/20 flex items-center justify-center group-hover:scale-110 group-hover:bg-cyan-500/30 transition-all duration-300">
                            <ShoppingBag class="w-4 h-4 text-cyan-300" />
                        </div>
                        <p class="text-lg sm:text-xl font-black text-white group-hover:text-cyan-200 transition-colors">{{ Math.round(counters.bookings) }}+</p>
                        <p class="text-[9px] text-white/60 uppercase tracking-wider group-hover:text-white/80 transition-colors">Booking</p>
                    </div>
                </div>

                <!-- Scroll Hint -->
                <button @click="scrollToDestinations" class="hero-item flex flex-col items-center gap-1.5 group cursor-pointer mx-auto">
