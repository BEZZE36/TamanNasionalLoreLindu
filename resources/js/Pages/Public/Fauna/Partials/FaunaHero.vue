<script setup>
/**
 * FaunaHero.vue - Premium Hero with GSAP Animations
 * Design: Follows DetailHero pattern with gradient background, animations
 */
import { ref, onMounted, onBeforeUnmount } from 'vue';
import { Link } from '@inertiajs/vue3';
import { gsap } from 'gsap';
import { Bird, Rabbit, Fish, ChevronRight, Shield, Eye } from 'lucide-vue-next';

const props = defineProps({
    totalFauna: { type: Number, default: 0 },
    totalEndemic: { type: Number, default: 0 },
    totalEndangered: { type: Number, default: 0 },
...
onMounted(() => {
    ctx = gsap.context(() => {
        ...
        tl.fromTo('.hero-badge', { opacity: 0, scale: 0.7, y: 10 }, { opacity: 1, scale: 1, y: 0, duration: 0.4, ease: 'back.out(3)' }, '-=0.2');
        tl.fromTo('.hero-title', { opacity: 0, y: 25, filter: 'blur(8px)' }, { opacity: 1, y: 0, filter: 'blur(0px)', duration: 0.5, ease: 'power3.out' }, '-=0.2');
        tl.fromTo('.hero-desc', { opacity: 0, y: 15 }, { opacity: 1, y: 0, duration: 0.4, ease: 'power2.out' }, '-=0.2');
        tl.fromTo('.hero-stat', { opacity: 0, scale: 0.8, y: 15 }, { opacity: 1, scale: 1, y: 0, duration: 0.4, stagger: 0.08, ease: 'back.out(2)' }, '-=0.1');
    }, heroRef.value);
});
...
<template>
    <section ref="heroRef" class="relative min-h-[45vh] sm:min-h-[50vh] overflow-hidden">
        ...
        <div class="absolute inset-0 pointer-events-none overflow-hidden">
            <div class="absolute top-1/4 right-[5%] w-48 sm:w-64 lg:w-80 h-48 sm:h-64 lg:h-80 bg-gradient-to-br from-amber-400/20 to-orange-400/10 rounded-full blur-3xl animate-pulse-slow"></div>
            <div class="absolute bottom-1/4 left-[5%] w-44 sm:w-56 lg:w-72 h-44 sm:h-56 lg:h-72 bg-gradient-to-tl from-orange-400/15 to-yellow-400/5 rounded-full blur-3xl animate-pulse-slow" style="animation-delay: 1s"></div>
            <div class="absolute top-[15%] right-[15%] w-16 h-16 border border-white/5 rounded-full animate-float"></div>
            <div class="absolute bottom-[30%] left-[10%] w-12 h-12 border border-amber-400/10 rounded-xl rotate-12 animate-float-delayed"></div>
        </div>
        ...
        <div class="relative z-10 max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 pt-24 sm:pt-28 pb-20 sm:pb-24">
            <nav class="hero-breadcrumb flex items-center gap-1.5 text-[10px] sm:text-[11px] text-white/60 mb-4 sm:mb-5 font-medium">
                ...
                <span class="text-white/90">Fauna</span>
            </nav>
            ...
            <div class="max-w-2xl">
                <div class="hero-badge inline-flex items-center gap-1.5 sm:gap-2 px-3 sm:px-4 py-1.5 sm:py-2 rounded-xl bg-white/10 backdrop-blur-xl border border-white/15 text-white mb-3 sm:mb-4 shadow-lg">
                    <span class="relative flex h-1.5 w-1.5 sm:h-2 sm:w-2">
                        ...
                        <span class="relative inline-flex rounded-full h-1.5 w-1.5 sm:h-2 sm:w-2 bg-amber-400"></span>
                    </span>
                    <Bird class="w-3 h-3 sm:w-3.5 sm:h-3.5" />
                    <span class="text-[10px] sm:text-[11px] font-semibold tracking-wide">Fauna Taman Nasional</span>
                </div>

                <h1 class="hero-title text-2xl sm:text-3xl md:text-4xl lg:text-5xl font-black text-white leading-tight mb-3 sm:mb-4">
                    Kekayaan Fauna
                    <span class="block mt-1 bg-gradient-to-r from-amber-200 via-orange-200 to-yellow-200 bg-clip-text text-transparent">
                        🦜 Lore Lindu
                    </span>
                </h1>

                <p class="hero-desc text-white/70 text-xs sm:text-sm max-w-lg mb-5 sm:mb-6 leading-relaxed">
                    Temukan keajaiban fauna endemik dan langka di Taman Nasional Lore Lindu, habitat bagi ratusan spesies satwa unik Sulawesi.
                </p>

                <div class="flex flex-wrap items-center gap-2 sm:gap-3">
                    <div class="hero-stat inline-flex items-center gap-1.5 px-3 py-2 rounded-xl bg-white/95 text-gray-900 shadow-lg">
                        <Rabbit class="w-3.5 h-3.5 text-amber-600" />
                        <span class="text-[11px] sm:text-xs font-bold">{{ formatCount(totalFauna) }}</span>
                    ...
                    <div class="hero-stat inline-flex items-center gap-1.5 px-3 py-2 rounded-xl bg-gradient-to-r from-purple-500 to-violet-500 text-white shadow-lg shadow-purple-500/25">
                        <Bird class="w-3.5 h-3.5" />
                        <span class="text-[11px] sm:text-xs font-bold">{{ formatCount(totalEndemic) }}</span>
                        <span class="text-[9px] sm:text-[10px] text-white/80">Endemik</span>
                    </div>
                    
                    <div class="hero-stat inline-flex items-center gap-1.5 px-3 py-2 rounded-xl bg-gradient-to-r from-red-500 to-rose-500 text-white shadow-lg shadow-red-500/25">
                        <Shield class="w-3.5 h-3.5" />
                        <span class="text-[11px] sm:text-xs font-bold">{{ formatCount(totalEndangered) }}</span>
                        <span class="text-[9px] sm:text-[10px] text-white/80">Dilindungi</span>
                    </div>
