<script setup>
/**
 * NewsHero.vue - Premium Hero matching Destination/Tiket Saya Design
...
import { Megaphone, Eye, Heart, MessageCircle, ChevronDown, Bell } from 'lucide-vue-next';
...
const counters = ref({ news: 0, views: 0, comments: 0 });
let ctx;

const scrollToContent = () => {
    gsap.to(window, { duration: 1, scrollTo: { y: '#news-content', offsetY: 80 }, ease: 'power3.inOut' });
};
...
<template>
    <section ref="heroRef" class="relative min-h-[70vh] flex items-center justify-center overflow-hidden pt-16 pb-12">
        <!-- Premium Gradient Background (Rose theme) -->
        <div ref="backgroundRef" class="absolute inset-0">
            <div class="absolute inset-0 bg-gradient-to-br from-slate-950 via-rose-950 to-slate-900"></div>
            ...
            <div class="absolute inset-0 opacity-60">
                ...
                <div class="absolute top-0 right-0 w-full h-full bg-[radial-gradient(ellipse_at_80%_60%,rgba(239,68,68,0.12),transparent_50%)]"></div>
                <div class="absolute bottom-0 left-1/2 w-full h-full bg-[radial-gradient(ellipse_at_50%_100%,rgba(251,113,133,0.08),transparent_50%)]"></div>
            </div>
            ...
            <div class="absolute inset-0 overflow-hidden pointer-events-none">
                ...
                <div class="float-shape absolute top-[25%] right-[12%] w-14 h-14 border border-rose-500/10 rounded-full"></div>
                <div class="float-shape absolute top-[55%] left-[15%] w-10 h-10 bg-gradient-to-br from-red-500/5 to-rose-500/5 rounded-lg rotate-45"></div>
                <div class="float-shape absolute bottom-[25%] right-[8%] w-16 h-16 border border-red-500/[0.08] rounded-xl -rotate-12"></div>
                <div class="float-shape absolute top-[40%] left-[60%] w-12 h-12 border border-rose-500/10 rounded-lg rotate-6"></div>
                <div class="float-shape absolute bottom-[40%] left-[25%] w-8 h-8 bg-gradient-to-br from-rose-500/5 to-red-500/5 rounded-full"></div>
            </div>
        ...
        <div ref="contentRef" class="relative z-10 max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="text-center max-w-3xl mx-auto">
                ...
                <div class="hero-item inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-white/10 backdrop-blur-md border border-white/20 text-white/90 mb-4 hover:bg-white/15 hover:border-white/30 hover:scale-105 transition-all duration-300">
                    ...
                    <span class="text-[10px] font-semibold tracking-wide">Update Terbaru</span>
                ...
                <h1 class="hero-item text-xl sm:text-2xl md:text-3xl lg:text-4xl font-black bg-gradient-to-r from-rose-300 via-red-200 to-pink-300 bg-clip-text text-transparent mb-4">
                    Terkini
                </h1>

                <!-- Description -->
                <p class="hero-item text-xs sm:text-sm text-white/60 max-w-xl mx-auto mb-8 leading-relaxed">
                    Informasi dan berita terbaru seputar Taman Nasional Lore Lindu. 
                    Update kegiatan, event, dan pengumuman penting.
                </p>

                <!-- Stats Cards -->
                <div class="hero-item grid grid-cols-3 gap-3 max-w-md mx-auto mb-8">
                    <!-- Total Berita -->
                    <div class="group bg-white/10 backdrop-blur-md border border-white/15 rounded-xl p-3 hover:bg-rose-500/15 hover:border-rose-400/30 hover:-translate-y-1 hover:shadow-lg hover:shadow-rose-500/10 transition-all duration-300 cursor-default">
                        <div class="w-8 h-8 mx-auto mb-1.5 rounded-lg bg-rose-500/20 flex items-center justify-center group-hover:scale-110 group-hover:bg-rose-500/30 transition-all duration-300">
                            <Megaphone class="w-4 h-4 text-rose-300" />
                        </div>
                        <p class="text-lg sm:text-xl font-black text-white group-hover:text-rose-200 transition-colors">{{ Math.round(counters.news) }}</p>
                        <p class="text-[7px] sm:text-[8px] text-white/60 uppercase tracking-wider group-hover:text-white/80 transition-colors">Berita</p>
                    </div>

                    <!-- Total Views -->
                    <div class="group bg-white/10 backdrop-blur-md border border-white/15 rounded-xl p-3 hover:bg-red-500/15 hover:border-red-400/30 hover:-translate-y-1 hover:shadow-lg hover:shadow-red-500/10 transition-all duration-300 cursor-default">
                        <div class="w-8 h-8 mx-auto mb-1.5 rounded-lg bg-red-500/20 flex items-center justify-center group-hover:scale-110 group-hover:bg-red-500/30 transition-all duration-300">
                            <Eye class="w-4 h-4 text-red-300" />
                        </div>
                        <p class="text-lg sm:text-xl font-black text-white group-hover:text-red-200 transition-colors">{{ Math.round(counters.views) }}+</p>
                    ...
                    <div class="group bg-white/10 backdrop-blur-md border border-white/15 rounded-xl p-3 hover:bg-pink-500/15 hover:border-pink-400/30 hover:-translate-y-1 hover:shadow-lg hover:shadow-pink-500/10 transition-all duration-300 cursor-default">
                        <div class="w-8 h-8 mx-auto mb-1.5 rounded-lg bg-pink-500/20 flex items-center justify-center group-hover:scale-110 group-hover:bg-pink-500/30 transition-all duration-300">
                            <MessageCircle class="w-4 h-4 text-pink-300" />
                        </div>
                        <p class="text-lg sm:text-xl font-black text-white group-hover:text-pink-200 transition-colors">{{ Math.round(counters.comments) }}</p>
