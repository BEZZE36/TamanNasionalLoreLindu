<script setup>
/**
 * Blog Detail Page - Premium Redesign
...
<template>
    <div ref="heroRef">
        <Head>
            <title>{{ article.title }} - Blog TNLL</title>
        ...
        <section class="relative min-h-[55vh] overflow-hidden">
            ...
            <div class="absolute inset-0">
                ...
                <div class="absolute inset-0 bg-gradient-to-r from-cyan-900/30 to-transparent"></div>
            </div>

            <!-- Floating Decorative Elements -->
            <div class="absolute inset-0 pointer-events-none overflow-hidden">
                <div class="absolute top-[15%] right-[10%] w-24 h-24 border border-white/5 rounded-full animate-pulse-slow"></div>
                <div class="absolute bottom-[25%] left-[5%] w-16 h-16 border border-cyan-500/10 rounded-2xl rotate-12 animate-float"></div>
                <div class="absolute top-[40%] right-[25%] w-8 h-8 bg-cyan-500/10 rounded-full blur-xl animate-pulse"></div>
            </div>
            ...
            <div class="relative z-10 max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 pt-24 sm:pt-28 pb-28 sm:pb-32">
                ...
                <div class="hero-badges flex flex-wrap items-center gap-2 mb-4">
                    ...
                    <span class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-white/95 text-gray-900 text-[10px] sm:text-xs font-bold shadow-lg">
                        <Clock class="w-3 h-3 text-cyan-500 animate-pulse" />
                        <span>Dibaca:</span>
                        <span class="text-cyan-600">{{ liveReadingTime }}</span>
                    </span>
                </div>
                ...
                <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 lg:gap-8">
                    ...
                    <div class="lg:col-span-2 space-y-6">
                        ...
                        <div v-if="article.tags?.length > 0" class="content-card flex flex-wrap gap-2">
                            <Link v-for="tag in article.tags" :key="tag.id" :href="`/blog?tag=${tag.slug}`"
                                class="inline-flex items-center gap-1.5 px-3 py-1.5 bg-cyan-50 text-cyan-700 rounded-full text-xs sm:text-sm font-medium hover:bg-cyan-100 hover:shadow-md transition-all duration-300">
                                <span class="text-cyan-400">#</span>{{ tag.name }}
                            </Link>
                        </div>
                        ...
                        <div class="content-card bg-white/80 backdrop-blur-xl rounded-2xl shadow-lg shadow-gray-200/50 border border-gray-100/80 p-5 sm:p-6">
                            ...
                            <div v-if="isLoggedIn" class="mb-6">
                                <div class="flex items-start gap-3">
                                    <div class="w-9 h-9 sm:w-10 sm:h-10 rounded-full bg-gradient-to-br from-cyan-500 to-blue-600 flex items-center justify-center text-white font-bold text-sm flex-shrink-0">
                                        {{ auth.user?.name?.[0] || 'U' }}
                                    </div>
                                    <div class="flex-1">
                                        <textarea v-model="commentText" rows="3" placeholder="Tulis komentar Anda..."
                                            class="w-full px-3 sm:px-4 py-2.5 sm:py-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-cyan-500/20 focus:border-cyan-500 resize-none text-xs sm:text-sm transition-all"></textarea>
                                        <div class="flex justify-end mt-2">
                                            <button @click="submitComment()" :disabled="isSubmitting || !commentText.trim()"
                                                class="inline-flex items-center gap-2 px-4 py-2 bg-gradient-to-r from-cyan-500 to-blue-600 text-white text-xs sm:text-sm font-semibold rounded-xl hover:shadow-lg hover:shadow-cyan-500/25 transition-all disabled:opacity-50 disabled:cursor-not-allowed">
                            ...
                            <div v-else class="mb-6 p-4 bg-gradient-to-r from-cyan-50 to-blue-50 rounded-xl text-center border border-cyan-100">
                                <p class="text-gray-600 text-xs sm:text-sm mb-2">Silakan login untuk mengirim komentar</p>
                                <Link href="/login" class="inline-flex items-center gap-1.5 text-cyan-600 font-semibold hover:underline text-xs sm:text-sm">
                                    Login sekarang
                                ...
                                <div v-for="comment in localComments" :key="comment.id" 
                                    ...
                                    <div class="flex items-start gap-2.5 sm:gap-3">
                                        ...
                                        <div class="flex-1 min-w-0">
                                            ...
                                            <p class="text-gray-700 text-xs sm:text-sm mb-2 leading-relaxed" v-html="comment.content.replace(/@(\w+)/g, '<span class=\'text-cyan-600 font-semibold\'>@$1</span>')"></p>
                                            
                                            <div class="flex items-center gap-3">
                                                <button v-if="isLoggedIn" @click="startReply(comment)"
                                                    class="inline-flex items-center gap-1 text-[10px] sm:text-xs text-gray-500 hover:text-cyan-600 transition-colors font-medium">
                                            ...
                                            <div v-if="replyingTo === comment.id" class="mt-3 flex items-start gap-2">
                                                <textarea v-model="replyText" rows="2" :placeholder="`Balas ke @${replyingToUser}...`"
                                                    class="flex-1 px-3 py-2 border border-gray-200 rounded-lg focus:ring-2 focus:ring-cyan-500/20 focus:border-cyan-500 resize-none text-xs sm:text-sm"></textarea>
                                                <div class="flex flex-col gap-1">
                                                    <button @click="submitComment(comment.id)" :disabled="isSubmitting || !replyText.trim()"
                                                        class="px-3 py-2 bg-cyan-500 text-white text-[10px] sm:text-xs font-semibold rounded-lg hover:bg-cyan-600 transition-colors disabled:opacity-50">
                                            ...
                                            <!-- Replies -->
                                            <div v-if="expandedReplies[comment.id] && comment.replies?.length > 0" class="mt-3 space-y-3 pl-3 sm:pl-4 border-l-2 border-cyan-200">
                                                <div v-for="reply in comment.replies" :key="reply.id" class="flex items-start gap-2">
                                                    ...
                                                    <div class="flex-1 min-w-0">
                                                        ...
                                                        <p class="text-gray-700 text-[10px] sm:text-xs leading-relaxed" v-html="reply.content.replace(/@(\w+)/g, '<span class=\'text-cyan-600 font-semibold\'>@$1</span>')"></p>
                                                        <!-- Reply to Reply Button -->
                                                        <button v-if="isLoggedIn" @click="startReply(reply, comment.id)"
                                                            class="inline-flex items-center gap-1 text-[9px] sm:text-[10px] text-gray-400 hover:text-cyan-600 transition-colors font-medium mt-1">
                                                            <Reply class="w-2.5 h-2.5" />Balas
                                                        </button>
                                                        <!-- Reply Form for Reply -->
                                                        <div v-if="replyingTo === reply.id" class="mt-2 flex items-start gap-2">
                                                            <textarea v-model="replyText" rows="2" :placeholder="`Balas ke @${replyingToUser}...`"
                                                                class="flex-1 px-2 py-1.5 border border-gray-200 rounded-lg focus:ring-2 focus:ring-cyan-500/20 focus:border-cyan-500 resize-none text-[10px] sm:text-xs"></textarea>
                                                            <div class="flex flex-col gap-1">
                                                                <button @click="submitComment(reply.id)" :disabled="isSubmitting || !replyText.trim()"
                                                                    class="px-2 py-1.5 bg-cyan-500 text-white text-[9px] sm:text-[10px] font-semibold rounded-lg hover:bg-cyan-600 transition-colors disabled:opacity-50">
                        ...
                        <!-- Related Articles -->
                        <div v-if="relatedArticles?.length > 0" class="content-card pt-6 sm:pt-8">
                            ...
                            <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 sm:gap-6">
                                <Link v-for="item in relatedArticles" :key="item.id" :href="`/blog/${item.slug}`" 
                                    ...
                                    <div class="p-3 sm:p-4">
                                        <h3 class="font-bold text-gray-900 text-xs sm:text-sm group-hover:text-cyan-600 transition-colors line-clamp-2">{{ item.title }}</h3>
                    ...
                    <div class="space-y-6">
                        <div class="sidebar-card bg-white/80 backdrop-blur-xl rounded-2xl shadow-lg shadow-gray-200/50 border border-gray-100/80 p-5 sm:p-6 sticky top-24">
                            ...
                            <div class="mb-6">
                                ...
                                <div class="flex items-center gap-3">
                                    <div class="w-12 h-12 sm:w-14 sm:h-14 rounded-xl bg-gradient-to-br from-cyan-100 to-cyan-200 flex items-center justify-center text-cyan-600 font-bold text-base sm:text-lg shadow-lg shadow-cyan-100">
                            ...
                            <!-- Excerpt -->
                            <div v-if="article.excerpt" class="mb-6 p-4 bg-gradient-to-br from-cyan-50 to-blue-50 rounded-xl border border-cyan-100/50">
                                <p class="text-xs sm:text-sm text-gray-600 italic leading-relaxed line-clamp-4">{{ article.excerpt }}</p>
                            </div>

                            <hr class="border-gray-200 my-6">
                            
                            <Link href="/blog" class="w-full py-3 flex items-center justify-center gap-2 bg-gradient-to-r from-cyan-500 to-blue-600 text-white font-bold text-xs sm:text-sm rounded-xl shadow-lg shadow-cyan-500/30 hover:shadow-cyan-500/50 hover:-translate-y-0.5 transition-all">
