...
import {
    FileText, Eye, Heart, MessageCircle, Clock, Star,
...
const changePeriod = () => {
    router.get('/admin/articles/dashboard', { period: period.value }, { preserveState: true });
};
...
// Chart computations with safe defaults
    ...
    <div class="min-h-screen space-y-5">
        ...
        <div class="relative overflow-hidden rounded-2xl bg-gradient-to-r from-indigo-600 via-violet-600 to-purple-600 p-6 shadow-2xl">
            ...
            <div class="header-content relative flex flex-col lg:flex-row items-start lg:items-center justify-between gap-4">
                <div class="flex items-center gap-4">
                    ...
                    <div>
                        <div class="flex items-center gap-2 mb-1">
                            <h1 class="text-xl font-bold text-white tracking-tight drop-shadow-lg">Dashboard Artikel</h1>
                        ...
                        <p class="text-indigo-100/80 text-xs">Statistik dan analitik artikel Anda</p>
                    </div>
                </div>
                ...
                <div class="flex items-center gap-3">
                    ...
                    <Link href="/admin/articles" class="group inline-flex items-center gap-2 px-5 py-2.5 rounded-xl bg-white text-indigo-600 text-xs font-bold hover:bg-indigo-50 hover:shadow-xl hover:-translate-y-0.5 transition-all duration-300 shadow-lg">
        ...
        <div class="grid grid-cols-2 lg:grid-cols-5 gap-3">
            <div class="stat-card group relative overflow-hidden rounded-xl bg-white p-4 shadow-lg border border-gray-100 hover:shadow-xl hover:-translate-y-1 transition-all duration-300">
                <div class="absolute top-0 right-0 w-20 h-20 bg-gradient-to-br from-indigo-100 to-violet-100 rounded-full -translate-y-1/2 translate-x-1/2 opacity-50 group-hover:opacity-100 transition-opacity"></div>
                <div class="relative flex items-center gap-3">
                    <div class="flex h-10 w-10 items-center justify-center rounded-xl bg-gradient-to-br from-indigo-500 to-violet-600 shadow-lg shadow-indigo-500/30 group-hover:scale-110 transition-transform">
                        <FileText class="w-5 h-5 text-white" />
                    </div>
                    <div>
                        <p class="text-xl font-black text-gray-900">{{ formatNumber(stats?.total || 0) }}</p>
                        <p class="text-[10px] text-gray-500 font-medium">Total Artikel</p>
                    </div>
                </div>
            </div>
        ...
        <div class="grid lg:grid-cols-2 gap-5">
            ...
            <div class="chart-section rounded-xl bg-white p-5 shadow-lg border border-gray-100">
                <h3 class="text-sm font-bold text-gray-900 mb-4 flex items-center gap-2">
                    <div class="flex h-8 w-8 items-center justify-center rounded-lg bg-gradient-to-br from-indigo-500 to-violet-600 shadow-md">
                ...
                <div v-if="viewsData.length > 0" class="h-48 flex items-end gap-1 px-2">
                    <div v-for="(val, i) in viewsData" :key="i" 
                        class="flex-1 bg-gradient-to-t from-indigo-500 to-violet-400 rounded-t-md hover:from-indigo-600 hover:to-violet-500 transition-all cursor-pointer relative group min-w-[8px]"
                ...
                <!-- X-axis labels -->
                <div v-if="viewsLabels.length > 0 && viewsLabels.length <= 14" class="flex justify-between mt-2 px-2">
                    <span v-for="(label, i) in viewsLabels" :key="i" class="text-[9px] text-gray-400 truncate flex-1 text-center">
                        {{ label }}
                    </span>
                </div>
            </div>
            ...
            <div class="chart-section rounded-xl bg-white p-5 shadow-lg border border-gray-100">
                ...
                <div v-if="peakLabels.length === 24" class="flex justify-between mt-2 px-2">
                    <span class="text-[9px] text-gray-400">00:00</span>
                    <span class="text-[9px] text-gray-400">06:00</span>
                    <span class="text-[9px] text-gray-400">12:00</span>
                    <span class="text-[9px] text-gray-400">18:00</span>
                    <span class="text-[9px] text-gray-400">23:00</span>
                </div>
            </div>
        </div>
        ...
        <div class="grid lg:grid-cols-3 gap-5">
            ...
            <div class="bottom-section rounded-xl bg-white p-5 shadow-lg border border-gray-100">
                ...
                <div class="space-y-2.5 max-h-72 overflow-y-auto">
                    <div v-for="(article, i) in topArticles" :key="article.id" class="flex items-center gap-3 group p-2 rounded-lg hover:bg-gray-50 transition-colors">
                        <span class="w-6 h-6 bg-gradient-to-br from-indigo-500 to-violet-600 text-white rounded-full flex items-center justify-center text-[10px] font-bold shadow-sm">{{ i + 1 }}</span>
                        <div class="flex-1 min-w-0">
                            <Link :href="`/admin/articles/${article.id}/edit`" class="text-xs font-medium text-gray-900 group-hover:text-indigo-600 truncate block transition-colors">
                                {{ article.title }}
                            </Link>
                            <p class="text-[10px] text-gray-400">{{ formatNumber(article.views_count) }} views</p>
                        </div>
                    </div>
                    <p v-if="!topArticles?.length" class="text-gray-400 text-xs text-center py-4">Belum ada data</p>
                </div>
            </div>
            ...
            <div class="bottom-section space-y-3">
                ...
                <div class="rounded-xl bg-white p-4 shadow-lg border border-gray-100">
                    ...
                    <p class="text-xl font-black bg-gradient-to-r from-indigo-600 to-violet-600 bg-clip-text text-transparent">{{ formatSeconds(avgReadTime) }}</p>
                </div>
