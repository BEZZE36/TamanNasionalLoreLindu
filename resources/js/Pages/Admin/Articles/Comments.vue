...
// Methods
const search = () => {
    router.get('/admin/articles/comments', { search: searchQuery.value, article_id: filterArticle.value }, { preserveState: true });
};
...
const submitComment = async () => {
    ...
    if (!replyToId.value && !newComment.value.article_id) return alert('Pilih artikel terlebih dahulu');
    
    isSubmitting.value = true;
    try {
        const url = replyToId.value 
            ? `/admin/articles/comments/${replyToId.value}/reply`
...
const confirmDelete = async () => {
    try {
        if (deleteType.value === 'single') {
            await fetch(`/admin/articles/comments/${deleteId.value}`, {
                method: 'DELETE',
                headers: { 'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content }
            });
        } else {
            await fetch('/admin/articles/comments/batch-delete', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content
...
const toggleStatus = async (id) => {
    await fetch(`/admin/articles/comments/${id}/toggle-status`, {
        method: 'POST',
        headers: { 'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content }
    });
    router.reload();
...
const togglePin = async (id) => {
    await fetch(`/admin/articles/comments/${id}/toggle-pin`, {
        method: 'POST',
        headers: { 'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content }
    });
    router.reload();
...
const toggleReplyStatus = async (replyId) => {
    try {
        await fetch(`/admin/articles/comments/${replyId}/toggle-status`, {
            method: 'POST',
            headers: { 'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content }
        });
        router.reload();
...
const deleteReply = async (replyId) => {
    ...
    try {
        await fetch(`/admin/articles/comments/${replyId}`, {
            method: 'DELETE',
            headers: { 'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content }
        });
        router.reload();
                ...
                <div class="flex items-center gap-4">
                    ...
                    <div>
                        <div class="flex items-center gap-2 mb-1">
                            <h1 class="text-xl font-bold text-white tracking-tight drop-shadow-lg">Komentar Artikel</h1>
                            <span class="px-2 py-0.5 bg-white/20 rounded-full text-[10px] font-bold text-white/90 backdrop-blur-sm">
                                {{ stats.total }} Total
                            </span>
                        </div>
                        <p class="text-indigo-100/80 text-xs">Kelola dan moderasi komentar artikel</p>
                    </div>
                </div>
                
                <div class="flex items-center gap-3">
                    <Link href="/admin/articles" class="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl bg-white/10 text-white text-xs font-semibold backdrop-blur-sm border border-white/20 hover:bg-white/20 transition-all">
        ...
        <div class="rounded-xl bg-white p-4 shadow-lg border border-gray-100">
            <div class="flex flex-col lg:flex-row gap-3">
                ...
                <select v-model="filterArticle" @change="search"
                    class="px-4 py-2.5 text-xs rounded-xl border-2 border-gray-200 bg-gray-50/50 focus:border-indigo-500 focus:ring-4 focus:ring-indigo-500/10 transition-all cursor-pointer">
                    <option value="">Semua Artikel</option>
                    <option v-for="a in articleList" :key="a.id" :value="a.id">{{ a.title }}</option>
                </select>
                <button @click="search" class="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl bg-gradient-to-r from-indigo-500 to-violet-600 text-white text-xs font-semibold hover:shadow-lg transition-all">
        ...
        <div class="rounded-xl bg-white shadow-lg border border-gray-100 overflow-hidden">
            <div v-if="comments?.data?.length" class="divide-y divide-gray-100">
                <div v-for="comment in comments.data" :key="comment.id"
                    :class="['comment-item p-4 hover:bg-gradient-to-r hover:from-indigo-50/50 hover:to-violet-50/30 transition-all', comment.is_pinned ? 'bg-violet-50/30' : '']">
                    <div class="flex items-start gap-3">
                        <input type="checkbox" :value="comment.id" v-model="selectedIds"
                            class="mt-1 w-4 h-4 rounded border-gray-300 text-indigo-500 focus:ring-indigo-500">
                        
                        <div class="w-10 h-10 rounded-full bg-gradient-to-br from-indigo-500 to-violet-600 flex items-center justify-center text-white font-bold text-xs flex-shrink-0 shadow-md">
                        ...
                        <div class="flex-1 min-w-0">
                            ...
                            <div v-if="comment.replies?.length" class="mt-4 space-y-3 pl-4 border-l-2 border-indigo-200">
            ...
            <div v-else class="text-center py-16">
                <div class="w-16 h-16 bg-gradient-to-br from-indigo-100 to-violet-100 rounded-2xl flex items-center justify-center mx-auto mb-4">
                    <MessageCircle class="w-8 h-8 text-indigo-400" />
                </div>
            ...
            <div v-if="comments?.links" class="px-5 py-4 border-t border-gray-100 flex justify-center gap-1 bg-gray-50/50">
                <Link v-for="link in comments.links" :key="link.label" :href="link.url || '#'"
                    :class="[
                        ...
                        link.active ? 'bg-gradient-to-r from-indigo-500 to-violet-600 text-white shadow-md' : 'text-gray-600 hover:bg-gray-100',
                ...
                <div v-if="showAddModal" class="fixed inset-0 z-50 flex items-center justify-center p-4">
                    ...
                    <div class="relative bg-white rounded-2xl shadow-2xl max-w-lg w-full p-6">
                        ...
                        <div v-if="!replyToId" class="mb-4">
                            <label class="block text-xs font-medium text-gray-700 mb-2">Artikel</label>
                            <select v-model="newComment.article_id" class="w-full px-4 py-2.5 text-xs rounded-xl border-2 border-gray-200 focus:border-indigo-500 focus:ring-4 focus:ring-indigo-500/10">
                        ...
                        <div class="mb-6">
                            ...
                            <textarea v-model="newComment.content" rows="4" placeholder="Tulis komentar..."
                                class="w-full px-4 py-3 text-xs rounded-xl border-2 border-gray-200 focus:border-indigo-500 focus:ring-4 focus:ring-indigo-500/10 resize-none"></textarea>
                        </div>
                        ...
                        <div class="flex gap-3 justify-end">
                            ...
                            <button @click="submitComment" :disabled="isSubmitting"
                                class="px-5 py-2.5 rounded-xl bg-gradient-to-r from-indigo-500 to-violet-600 text-white text-xs font-semibold hover:shadow-lg transition-all disabled:opacity-50 flex items-center gap-2">
