...
const props = defineProps({ categories: { type: Object, default: () => ({}) } });

const form = useForm({
    title: '', excerpt: '', content: '', featured_image: null, category: 'wisata',
        ...
        <div class="flex items-center gap-3">
            ...
            <div>
                <h1 class="text-lg font-bold text-gray-900 flex items-center gap-2">
                    <FileText class="w-7 h-7 text-rose-600" />Tulis Artikel Baru
                </h1>
                <p class="text-gray-500 text-sm">Buat konten artikel terbaru</p>
            </div>
        </div>
        ...
        <form @submit.prevent="submit" class="grid grid-cols-1 lg:grid-cols-3 gap-3">
            ...
            <div class="lg:col-span-2 space-y-6">
                ...
                <div class="bg-white rounded-2xl shadow-lg border border-gray-100 p-6">
                    ...
                    <TinyMceEditor v-model="form.content" id="article-content" :height="400" placeholder="Tulis konten artikel..." />
            ...
            <div class="space-y-4">
                ...
                <button type="submit" :disabled="form.processing"
                    class="w-full py-4 bg-gradient-to-r from-rose-500 to-pink-600 text-white font-bold text-lg rounded-xl shadow-lg shadow-rose-500/30 hover:shadow-rose-500/50 transition-all flex items-center justify-center gap-2 disabled:opacity-50">
                    <Save class="w-5 h-5" />{{ form.processing ? 'Menyimpan...' : 'Simpan Artikel' }}
                </button>
            </div>
        </form>
    </div>
