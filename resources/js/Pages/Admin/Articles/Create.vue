...
import { 
    FileText, ArrowLeft, Save, Image, Type, AlignLeft, Star, Eye, User, 
    Calendar, Search, Pin, Settings, Wand2, Loader2, AlertTriangle,
    Languages, Pencil, RefreshCw, Sparkles, Send
...
let ctx;
const showFloatingSave = ref(true); // Always visible
...
// Handle edited image from ImageEditorModal
...
const isFormValid = computed(() => {
    return form.title?.trim() && form.content?.trim() && form.category?.trim() &&
...
const generateTitle = async () => {
    ...
    try {
        const res = await fetch('/admin/ai/headlines', {
            ...
            body: JSON.stringify({ content: form.content.replace(/<[^>]*>/g, '').substring(0, 2000), count: 1 })
        });
...
// Open topic modal for title generation
const openTopicModal = () => { topicInput.value = ''; showTopicModal.value = true; };

// Generate title from topic
const confirmGenerateTitle = async () => {
    if (!topicInput.value?.trim()) { alert('Masukkan topik terlebih dahulu'); return; }
    showTopicModal.value = false;
    aiLoadingTitle.value = true;
    try {
        const res = await fetch('/admin/ai/headlines', {
            method: 'POST', headers: { 'Content-Type': 'application/json', 'X-CSRF-TOKEN': csrfToken() },
            body: JSON.stringify({ content: topicInput.value.trim(), count: 1 })
        });
        const data = await res.json();
        if (data.success && data.content) {
            const match = data.content.match(/1\.\s*(.+)/);
...
const translateTitle = async (direction) => {
    if (!form.title?.trim()) { alert('Tulis judul terlebih dahulu'); return; }
    aiTranslatingTitle.value = true;
    try {
        const res = await fetch('/admin/ai/translate', {
            method: 'POST', headers: { 'Content-Type': 'application/json', 'X-CSRF-TOKEN': csrfToken() },
            body: JSON.stringify({ content: form.title, direction })
        });
        const data = await res.json();
        if (data.success && data.content) form.title = data.content.trim();
    } catch (e) { console.error(e); }
...
const translateExcerpt = async (direction) => {
    if (!form.excerpt?.trim()) { alert('Tulis ringkasan terlebih dahulu'); return; }
    aiTranslatingExcerpt.value = true;
    try {
        const res = await fetch('/admin/ai/translate', {
            method: 'POST', headers: { 'Content-Type': 'application/json', 'X-CSRF-TOKEN': csrfToken() },
            body: JSON.stringify({ content: form.excerpt, direction })
        });
        const data = await res.json();
        if (data.success && data.content) form.excerpt = data.content.trim();
    } catch (e) { console.error(e); }
...
<template>
    <div class="max-w-6xl mx-auto space-y-4">
        <!-- Premium Header with Quick Publish -->
        <div class="relative overflow-hidden rounded-xl bg-gradient-to-r from-indigo-600 via-violet-600 to-purple-600 p-4 shadow-xl">
            ...
            <div class="header-content relative flex flex-col lg:flex-row items-start lg:items-center justify-between gap-3">
                <div class="flex items-center gap-3">
                    <Link href="/admin/articles" class="flex h-8 w-8 items-center justify-center rounded-lg bg-white/20 backdrop-blur-sm ring-1 ring-white/30 hover:bg-white/30 transition-all">
                        <ArrowLeft class="w-4 h-4 text-white" />
                    </Link>
                    <div>
                        <div class="flex items-center gap-2">
                            <h1 class="text-base font-bold text-white tracking-tight">Tulis Artikel Baru</h1>
                            <Sparkles class="w-3.5 h-3.5 text-amber-300" />
                        </div>
                        <p class="text-indigo-100/80 text-[10px]">Buat konten artikel terbaru</p>
                    </div>
                </div>
                <div class="flex items-center gap-2">
                    ...
                    <!-- Quick Publish Toggle in Header -->
                    <button type="button" @click="quickPublish" :disabled="!isFormValid || form.processing"
                        class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-[11px] font-bold transition-all disabled:opacity-50 disabled:cursor-not-allowed"
                        :class="isFormValid ? 'bg-white text-indigo-600 hover:bg-indigo-50 shadow-lg' : 'bg-white/20 text-white/60'">
                        <Send class="w-3 h-3" />
                        <span>Publish</span>
                    </button>
                </div>
            ...
            <div class="lg:col-span-2 space-y-4">
                ...
                <div class="form-section bg-white rounded-xl shadow-md border border-gray-100 p-4 hover:shadow-lg transition-shadow">
                    <h3 class="text-xs font-bold text-gray-900 mb-3 flex items-center gap-2">
                        <Type class="w-4 h-4 text-indigo-500" />Judul & Ringkasan
                    </h3>
                    <div class="space-y-3">
                        <div class="relative">
                            <div class="flex items-center justify-between mb-1">
                                <label class="text-[11px] font-medium text-gray-700">Judul Artikel *</label>
                                <div class="flex items-center gap-1">
                                    <button type="button" @click="translateTitle('id_to_en')" :disabled="aiTranslatingTitle"
                                        class="px-1.5 py-0.5 text-[9px] font-medium text-blue-600 hover:bg-blue-50 rounded transition-colors disabled:opacity-50">EN</button>
                                    <button type="button" @click="translateTitle('en_to_id')" :disabled="aiTranslatingTitle"
                                        class="px-1.5 py-0.5 text-[9px] font-medium text-green-600 hover:bg-green-50 rounded transition-colors disabled:opacity-50">ID</button>
                                    <button type="button" @click="openTopicModal" :disabled="aiLoadingTitle"
                                        class="inline-flex items-center gap-0.5 px-1.5 py-0.5 text-[9px] font-medium text-violet-600 hover:bg-violet-50 rounded transition-colors disabled:opacity-50">
                                        <Wand2 class="w-2.5 h-2.5" />AI
                                    </button>
                                </div>
                            </div>
                            ...
                            <input v-model="form.title" type="text" required placeholder="Masukkan judul artikel..."
                                :class="['w-full px-3 py-2 rounded-lg border focus:ring-2 text-sm font-medium transition-colors', 
                                    form.title?.trim() ? 'border-gray-200 focus:border-indigo-500 focus:ring-indigo-500/20' : 'border-red-300 focus:border-red-500 focus:ring-red-500/20']">
                        ...
                        <div class="relative">
                            <div class="flex items-center justify-between mb-1">
                                <label class="text-[11px] font-medium text-gray-700">Ringkasan/Excerpt *</label>
                                <div class="flex items-center gap-1">
                                    <button type="button" @click="translateExcerpt('id_to_en')" :disabled="aiTranslatingExcerpt"
                                        class="px-1.5 py-0.5 text-[9px] font-medium text-blue-600 hover:bg-blue-50 rounded transition-colors disabled:opacity-50">EN</button>
                                    <button type="button" @click="translateExcerpt('en_to_id')" :disabled="aiTranslatingExcerpt"
                                        class="px-1.5 py-0.5 text-[9px] font-medium text-green-600 hover:bg-green-50 rounded transition-colors disabled:opacity-50">ID</button>
                                    <button type="button" @click="generateExcerpt" :disabled="aiLoadingExcerpt"
                                        class="inline-flex items-center gap-0.5 px-1.5 py-0.5 text-[9px] font-medium text-violet-600 hover:bg-violet-50 rounded transition-colors disabled:opacity-50">
                                        <Wand2 class="w-2.5 h-2.5" />AI
                                    </button>
                                </div>
                            ...
                            <div v-if="aiLoadingExcerpt || aiTranslatingExcerpt" class="absolute inset-0 z-10 flex items-start pt-6 bg-white rounded-lg">
                                <div class="w-full px-3 space-y-1.5">
                                    <div class="h-3 w-full bg-gradient-to-r from-gray-200 via-gray-100 to-gray-200 rounded animate-pulse"></div>
                                    <div class="h-3 w-11/12 bg-gradient-to-r from-gray-200 via-gray-100 to-gray-200 rounded animate-pulse"></div>
                                </div>
                            </div>
                            <textarea v-model="form.excerpt" rows="2" placeholder="Ringkasan singkat artikel..."
                                class="w-full px-3 py-2 rounded-lg border border-gray-200 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/20 text-sm resize-none"></textarea>
                ...
                <div class="form-section bg-white rounded-xl shadow-md border border-gray-100 p-4 hover:shadow-lg transition-shadow">
                    <h3 class="text-xs font-bold text-gray-900 mb-3 flex items-center gap-2">
                        <AlignLeft class="w-4 h-4 text-indigo-500" />Konten Artikel *
                    </h3>
                    <TinyMceEditor v-model="form.content" id="article-content" :height="350" placeholder="Tulis konten artikel..." />
                    <p v-if="form.errors.content" class="text-red-500 text-[10px] mt-1">{{ form.errors.content }}</p>
                </div>
                ...
                <div class="form-section bg-white rounded-xl shadow-md border border-gray-100 overflow-hidden hover:shadow-lg transition-shadow">
                    <button type="button" @click="showSeoPanel = !showSeoPanel" 
                        class="w-full px-4 py-3 flex items-center justify-between hover:bg-gray-50 transition-colors">
                        <h3 class="text-xs font-bold text-gray-900 flex items-center gap-2">
                            <Search class="w-4 h-4 text-green-500" />SEO & Meta
                        </h3>
                    ...
                    <div v-if="showSeoPanel" class="px-4 pb-4 space-y-3 border-t border-gray-100 pt-3">
                        <button type="button" @click="generateSeo" :disabled="aiLoadingSeo"
                            class="w-full flex items-center justify-center gap-1.5 px-3 py-2 bg-gradient-to-r from-violet-500 to-fuchsia-500 text-white rounded-lg text-[11px] font-medium hover:opacity-90 transition-all disabled:opacity-50">
                            <Loader2 v-if="aiLoadingSeo" class="w-3 h-3 animate-spin" />
                        ...
                        <div class="relative">
                            ...
                            <input v-model="form.meta_title" type="text" placeholder="Judul untuk mesin pencari..."
                                class="w-full px-3 py-2 rounded-lg border border-gray-200 focus:border-green-500 focus:ring-2 focus:ring-green-500/20 text-sm">
                        ...
                        <div class="relative">
                            ...
                            <textarea v-model="form.meta_description" rows="2" placeholder="Deskripsi untuk hasil pencarian..."
                                class="w-full px-3 py-2 rounded-lg border border-gray-200 focus:border-green-500 focus:ring-2 focus:ring-green-500/20 text-sm resize-none"></textarea>
                        ...
                        <div class="relative">
                            ...
                            <input v-model="form.meta_keywords" type="text" placeholder="kata kunci, dipisah, koma..."
                                class="w-full px-3 py-2 rounded-lg border border-gray-200 focus:border-green-500 focus:ring-2 focus:ring-green-500/20 text-sm">
                        </div>
                    </div>
                </div>
            ...
            <div class="space-y-3">
                ...
                <div class="sidebar-section bg-white rounded-xl shadow-md border border-gray-100 p-3 hover:shadow-lg transition-shadow">
                    <h3 class="text-xs font-bold text-gray-900 mb-2 flex items-center gap-2">
                        <Image class="w-4 h-4 text-indigo-500" />Gambar Utama *
                    </h3>
                    <div class="space-y-2">
                        <div class="relative aspect-video rounded-lg overflow-hidden bg-gray-100 group">
                            ...
                            <div v-if="imagePreview" class="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-2">
                                <button type="button" @click="showImageEditor = true"
                                    class="flex items-center gap-1 px-2 py-1 bg-white/90 text-gray-800 rounded-md text-[10px] font-medium hover:bg-white transition-colors shadow">
                                    <Pencil class="w-3 h-3" />Edit
                                </button>
                                <label class="flex items-center gap-1 px-2 py-1 bg-indigo-500 text-white rounded-md text-[10px] font-medium hover:bg-indigo-600 transition-colors cursor-pointer shadow">
                                    <RefreshCw class="w-3 h-3" />Ganti
                                    <input type="file" accept=".jpg,.jpeg,.png,.webp" @change="handleImageChange" class="hidden">
                                </label>
                            </div>
                        </div>
                        <label v-if="!imagePreview" class="block w-full py-2 bg-indigo-50 text-indigo-600 font-medium text-center rounded-lg cursor-pointer hover:bg-indigo-100 transition-colors text-xs">
                            <input type="file" accept=".jpg,.jpeg,.png,.webp" @change="handleImageChange" class="hidden">
                            Pilih Gambar
                        </label>
                        <p v-if="form.errors.featured_image" class="text-red-500 text-[10px]">{{ form.errors.featured_image }}</p>
                    </div>
                </div>
                ...
                <div class="sidebar-section bg-white rounded-xl shadow-md border border-gray-100 p-3 hover:shadow-lg transition-shadow">
                    <h3 class="text-xs font-bold text-gray-900 mb-2 flex items-center gap-2">
                        <Settings class="w-4 h-4 text-gray-500" />Pengaturan
                    </h3>
                    <div class="space-y-2">
                        <div>
                            <label class="text-[11px] font-medium text-gray-700 mb-1 block">Kategori *</label>
                            <select v-model="form.category" required
                                class="w-full px-2 py-1.5 rounded-lg border border-gray-200 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/20 text-xs">
                                <option v-for="(label, key) in categories" :key="key" :value="key">{{ label }}</option>
                            </select>
                        </div>
                        <div>
                            <label class="text-[11px] font-medium text-gray-700 mb-1 flex items-center gap-1">
                                <User class="w-3 h-3" /> Penulis
                            </label>
                ...
                <div class="sidebar-section bg-white rounded-xl shadow-md border border-gray-100 p-3 hover:shadow-lg transition-shadow">
                    <h3 class="text-xs font-bold text-gray-900 mb-2 flex items-center gap-2">
                        <Calendar class="w-4 h-4 text-blue-500" />Penjadwalan
                    </h3>
                    <div>
                        <label class="text-[11px] font-medium text-gray-700 mb-1 block">Jadwal Publikasi</label>
                        <input v-model="form.scheduled_at" type="datetime-local"
                            class="w-full px-2 py-1.5 rounded-lg border border-gray-200 focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20 text-xs">
                ...
                <div class="sidebar-section bg-white rounded-xl shadow-md border border-gray-100 p-3 space-y-2 hover:shadow-lg transition-shadow">
                    <label class="flex items-center gap-2 p-2 bg-yellow-50 rounded-lg cursor-pointer hover:bg-yellow-100 transition-colors">
                        ...
                        <span class="font-medium text-yellow-700 flex items-center gap-1 text-[11px]">
                            <Star class="w-3 h-3" />Artikel Unggulan
                        </span>
                    </label>
                    <label class="flex items-center gap-2 p-2 bg-purple-50 rounded-lg cursor-pointer hover:bg-purple-100 transition-colors">
                        ...
                        <span class="font-medium text-purple-700 flex items-center gap-1 text-[11px]">
                            <Pin class="w-3 h-3" />Pin di Atas
                        </span>
                    </label>
                    <label class="flex items-center gap-2 p-2 bg-green-50 rounded-lg cursor-pointer hover:bg-green-100 transition-colors">
                        ...
                        <span class="font-medium text-green-700 flex items-center gap-1 text-[11px]">
                            <Eye class="w-3 h-3" />Publikasikan
                        </span>
                    </label>
                </div>
                ...
                <div v-if="!isFormValid" class="flex items-center gap-1.5 px-2 py-1.5 bg-amber-50 border border-amber-200 rounded-lg text-[10px] text-amber-700">
                    ...
                    <span>Lengkapi: Judul, Konten, Kategori, Ringkasan, Penulis, SEO</span>
                </div>
            </div>
        </form>
        ...
        <Transition name="float">
            <div v-if="showFloatingSave" class="fixed bottom-6 right-6 z-50 flex items-center gap-3">
                ...
                <button type="button" @click="submit" :disabled="form.processing || !isFormValid"
                    class="flex items-center gap-2 px-5 py-3 bg-gradient-to-r from-indigo-500 to-violet-600 text-white rounded-full shadow-2xl shadow-indigo-500/40 hover:shadow-indigo-500/60 hover:-translate-y-1 transition-all disabled:opacity-50 disabled:cursor-not-allowed">
                    <Save class="w-5 h-5" />
                    <span class="font-bold text-sm">{{ form.processing ? 'Menyimpan...' : 'Simpan Artikel' }}</span>
                </button>
            </div>
        </Transition>
                ...
                <div v-if="showTopicModal" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
                    <div class="bg-white rounded-xl shadow-2xl max-w-md w-full p-4">
                        <h3 class="text-sm font-bold text-gray-900 mb-3 flex items-center gap-2">
                            <Wand2 class="w-4 h-4 text-violet-600" />Masukkan Topik
                        </h3>
                        <p class="text-xs text-gray-500 mb-3">Masukkan topik atau ide untuk membuat judul artikel dengan AI</p>
                        <input v-model="topicInput" type="text" placeholder="Contoh: Wisata alam di Sulawesi Tengah..."
                            class="w-full px-3 py-2 rounded-lg border border-gray-200 focus:border-violet-500 focus:ring-2 focus:ring-violet-500/20 text-sm mb-3"
                            @keyup.enter="confirmGenerateTitle">
