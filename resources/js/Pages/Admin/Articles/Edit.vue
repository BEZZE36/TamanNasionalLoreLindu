...
import { 
    ...
    Calendar, Clock, Search, Tag, Pin, Archive, History, Settings,
    CheckCircle, AlertTriangle, Wand2, Loader2, Languages, Pencil, RefreshCw, ExternalLink, Sparkles
...
const form = useForm({
    ...
    featured_image: null,
    category: props.article.category || 'wisata',
    author_name: props.article.author_name || adminName.value,
    ...
    scheduled_at: props.article.scheduled_at ? props.article.scheduled_at.slice(0, 16) : '',
    ...
    locale: props.article.locale || 'id',
...
const revisions = ref([]);
const lastAutoSave = ref(props.article.last_auto_save || null);
...
const autoSave = async () => {
    ...
    try {
        const response = await fetch(`/admin/articles/${props.article.id}/auto-save`, {
            method: 'POST',
            headers: {
            ...
            body: JSON.stringify({
                title: form.title, content: form.content, excerpt: form.excerpt, category: form.category,
...
watch(() => [
    form.title, form.content, form.excerpt, form.category,
    form.author_name, form.is_featured, form.is_published, form.is_pinned,
...
const loadRevisions = async () => {
    try {
        const response = await fetch(`/admin/articles/${props.article.id}/revisions`);
        const data = await response.json();
...
const restoreRevision = async (revisionId) => {
    if (!confirm('Kembalikan ke revisi ini? Perubahan saat ini akan disimpan sebagai revisi baru.')) return;
    try {
        const response = await fetch(`/admin/articles/${props.article.id}/restore-revision/${revisionId}`, {
            method: 'POST',
...
const submit = () => {
    form.transform(data => ({ ...data, _method: 'PUT' })).post(`/admin/articles/${props.article.id}`, {
        forceFormData: true, onSuccess: () => router.visit('/admin/articles')
    });
};
...
const isFormValid = computed(() => form.title?.trim() && form.content?.trim() && form.category?.trim() &&
...
const generateTitle = async () => {
    if (!form.content?.trim()) { alert('Tulis konten terlebih dahulu untuk generate judul'); return; }
    ...
    try {
        ...
        if (handleAIResponse(data)) return;
...
const generateExcerpt = async () => {
    if (!form.content?.trim()) { alert('Tulis konten terlebih dahulu untuk generate ringkasan'); return; }
        ...
        <div class="relative overflow-hidden rounded-xl bg-gradient-to-r from-indigo-600 via-violet-600 to-purple-600 p-4 shadow-xl">
            ...
            <div class="header-content relative flex flex-col lg:flex-row items-start lg:items-center justify-between gap-3">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        <div class="flex items-center gap-2">
                            <h1 class="text-base font-bold text-white tracking-tight">Edit Artikel</h1>
                            <Sparkles class="w-3.5 h-3.5 text-amber-300" />
                        </div>
                        <p class="text-indigo-100/80 text-[10px] line-clamp-1 max-w-md">{{ article.title }}</p>
                    </div>
                </div>
                <div class="flex items-center gap-2 flex-wrap">
                    ...
                    <a v-if="article.is_published" :href="`/blog/${article.slug}`" target="_blank" 
                       class="inline-flex items-center gap-1 px-2 py-1 bg-white text-indigo-600 rounded-md text-[10px] font-bold hover:bg-indigo-50 transition-all shadow-md">
        ...
        <form @submit.prevent="submit" class="grid grid-cols-1 lg:grid-cols-3 gap-4">
            <div class="lg:col-span-2 space-y-4">
                <div class="form-section bg-white rounded-xl shadow-md border border-gray-100 p-4 hover:shadow-lg transition-shadow">
                    <h3 class="text-xs font-bold text-gray-900 mb-3 flex items-center gap-2"><Type class="w-4 h-4 text-indigo-500" />Judul & Ringkasan</h3>
                    <div class="space-y-3">
                        <div class="relative">
                            ...
                            <input v-model="form.title" type="text" required placeholder="Masukkan judul artikel..."
                                :class="['w-full px-3 py-2 rounded-lg border focus:ring-2 text-sm font-medium transition-colors', form.title?.trim() ? 'border-gray-200 focus:border-indigo-500 focus:ring-indigo-500/20' : 'border-red-300 focus:border-red-500 focus:ring-red-500/20']">
                        ...
                        <div class="relative">
                            ...
                            <textarea v-model="form.excerpt" rows="2" placeholder="Ringkasan singkat artikel..." class="w-full px-3 py-2 rounded-lg border border-gray-200 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/20 text-sm resize-none"></textarea>
                ...
                <div class="form-section bg-white rounded-xl shadow-md border border-gray-100 p-4 hover:shadow-lg transition-shadow">
                    <h3 class="text-xs font-bold text-gray-900 mb-3 flex items-center gap-2"><AlignLeft class="w-4 h-4 text-indigo-500" />Konten Artikel *</h3>
                    <TinyMceEditor v-model="form.content" id="article-content-edit" :height="350" placeholder="Tulis konten artikel..." />
            ...
            <div class="space-y-3">
                <div class="sidebar-section bg-gradient-to-br from-indigo-500 to-violet-600 rounded-xl p-3 text-white">
                    ...
                    <div class="text-[10px] opacity-80 space-y-0.5">
                        ...
                        <p v-if="form.is_published && article.published_at">Dipublikasikan: {{ formatDate(article.published_at) }}</p>
                ...
                <div class="sidebar-section bg-white rounded-xl shadow-md border border-gray-100 p-3 hover:shadow-lg transition-shadow">
                    <h3 class="text-xs font-bold text-gray-900 mb-2 flex items-center gap-2"><Image class="w-4 h-4 text-indigo-500" />Gambar Utama</h3>
                    <div class="space-y-2">
                        <div class="relative aspect-video rounded-lg overflow-hidden bg-gray-100 group">
                            ...
                            <div v-if="imagePreview" class="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-2">
                                ...
                                <label class="flex items-center gap-1 px-2 py-1 bg-indigo-500 text-white rounded-md text-[10px] font-medium hover:bg-indigo-600 transition-colors cursor-pointer shadow"><RefreshCw class="w-3 h-3" />Ganti<input type="file" accept=".jpg,.jpeg,.png,.webp" @change="handleImageChange" class="hidden"></label>
                            </div>
                        </div>
                ...
                <div class="sidebar-section bg-white rounded-xl shadow-md border border-gray-100 p-3 hover:shadow-lg transition-shadow">
                    ...
                    <div class="space-y-2">
                        <div>
                            ...
                            <select v-model="form.category" required class="w-full px-2 py-1.5 rounded-lg border border-gray-200 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/20 text-xs">
                        ...
                        <div>
                            ...
                            <input v-model="form.author_name" type="text" placeholder="Nama penulis..." class="w-full px-2 py-1.5 rounded-lg border border-gray-200 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/20 text-xs">
                        </div>
                    </div>
                </div>
                ...
                <div class="sidebar-section bg-white rounded-xl shadow-md border border-gray-100 p-3 space-y-2 hover:shadow-lg transition-shadow">
                    ...
                    <label class="flex items-center gap-2 p-2 bg-yellow-50 rounded-lg cursor-pointer hover:bg-yellow-100 transition-colors">
                        ...
                        <span class="font-medium text-yellow-700 flex items-center gap-1 text-[11px]"><Star class="w-3 h-3" />Artikel Unggulan</span>
                    </label>
                ...
                <div class="sidebar-section bg-white rounded-xl shadow-md border border-gray-100 p-3 hover:shadow-lg transition-shadow">
                    <button type="button" @click="loadRevisions" class="w-full flex items-center justify-center gap-2 py-1.5 text-xs font-medium text-gray-600 hover:text-indigo-600 transition-colors">
                        <History class="w-3.5 h-3.5" /> Riwayat Revisi ({{ article.revision_count || 0 }})
                    </button>
                </div>
                ...
                <div v-if="!isFormValid" class="flex items-center gap-1.5 px-2 py-1.5 bg-amber-50 border border-amber-200 rounded-lg text-[10px] text-amber-700">
                    <AlertTriangle class="w-3 h-3 flex-shrink-0" /><span>Lengkapi: Judul, Konten, Kategori, Ringkasan, Penulis, SEO</span>
                </div>
            </div>
        </form>
                            ...
                            <div v-for="rev in revisions" :key="rev.id" class="p-3 bg-gray-50 rounded-lg">
                                <div class="flex items-center justify-between">
                                    <div>
                                        ...
                                        <p v-if="rev.change_summary" class="text-[10px] text-gray-400 mt-0.5">{{ rev.change_summary }}</p>
                                    </div>
                                    <button @click="restoreRevision(rev.id)" class="px-2 py-1 text-[10px] font-medium text-indigo-600 bg-indigo-50 rounded-md hover:bg-indigo-100 transition-colors">Pulihkan</button>
                                </div>
                            </div>
        ...
        <Transition name="float">
            <div v-if="showFloatingSave" class="fixed bottom-6 right-6 z-50 flex items-center gap-3">
                ...
                <button type="button" @click="submit" :disabled="form.processing || !isFormValid"
                    ...
                    <Save class="w-5 h-5" /><span class="font-bold text-sm">{{ form.processing ? 'Menyimpan...' : 'Simpan Artikel' }}</span>
                </button>
            </div>
        </Transition>
    </div>
