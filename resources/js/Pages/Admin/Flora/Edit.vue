...
import { Flower2, Save, ArrowLeft, Upload, X, Plus, Info, Tag, FileText, Image, Sparkles, Pencil, Wand2, Loader2, CheckCircle, Clock, Eye } from 'lucide-vue-next';

defineOptions({ layout: AdminLayout });

const props = defineProps({ flora: { type: Object, required: true }, categories: { type: Object, default: () => ({}) }, conservationStatuses: { type: Object, default: () => ({}) } });

const form = useForm({
    name: props.flora.name || '', local_name: props.flora.local_name || '', scientific_name: props.flora.scientific_name || '',
    description: props.flora.description || '', category: props.flora.category || 'umum',
    conservation_status: props.flora.conservation_status || '', meta_title: props.flora.meta_title || '', meta_description: props.flora.meta_description || '',
    is_active: props.flora.is_active ?? true, is_featured: props.flora.is_featured ?? false, image: null, gallery: [], _method: 'PUT'
});
...
watch(() => [form.name, form.description, form.meta_title, form.meta_description], () => {
...
const deleteExistingImage = async (id) => { if (!confirm('Hapus gambar?')) return; try { await fetch(`/admin/flora/images/${id}`, { method: 'DELETE', headers: { 'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]')?.content } }); existingImages.value = existingImages.value.filter(img => img.id !== id); } catch (e) {} };
...
const submit = () => {
    ...
    router.post(`/admin/flora/${props.flora.id}`, fd, {
...
<template>
    <div class="min-h-screen space-y-4">
            ...
            <div class="header-content relative flex items-center justify-between">
                <div class="flex items-center gap-3">
                    ...
                    <div class="flex items-center gap-3">
                        ...
                        <div><h1 class="text-lg font-bold text-white flex items-center gap-2"><Flower2 class="w-5 h-5" />Edit Flora</h1><p class="text-emerald-100/80 text-xs truncate max-w-xs">{{ flora.name }}</p></div>
                    </div>
                </div>
                <div class="flex items-center gap-2">
                    ...
                    <a v-if="flora.is_active" :href="`/flora/${flora.slug || flora.id}`" target="_blank" class="p-2 rounded-xl bg-white/10 border border-white/20 hover:bg-white/20 transition-all"><Eye class="w-4 h-4 text-white" /></a>
                </div>
            </div>
        </div>
        ...
        <div class="form-section bg-white rounded-xl p-4 shadow-lg border border-gray-100 flex items-center justify-between">
            <div class="flex items-center gap-3">
                <div :class="['w-10 h-10 rounded-xl flex items-center justify-center', flora.is_active ? 'bg-emerald-100' : 'bg-amber-100']"><CheckCircle :class="['w-5 h-5', flora.is_active ? 'text-emerald-600' : 'text-amber-600']" /></div>
                <div><p class="text-xs font-bold text-gray-900">{{ flora.is_active ? 'Dipublikasi' : 'Draft' }}</p><p class="text-[10px] text-gray-500">{{ formatDate(flora.updated_at) }}</p></div>
            </div>
            <div class="flex items-center gap-2 text-[10px] text-gray-500"><Clock class="w-3.5 h-3.5" /><span>Dibuat {{ formatDate(flora.created_at) }}</span></div>
        </div>
        ...
        <form @submit.prevent="submit" class="grid grid-cols-1 xl:grid-cols-3 gap-4">
            <div class="xl:col-span-2 space-y-4">
                ...
                <div class="form-section bg-white rounded-2xl shadow-lg border border-gray-100 p-5">
                    ...
                    <div class="space-y-3">
                        <div><div class="flex items-center justify-between mb-1"><label class="text-[11px] font-medium text-gray-700">Nama Flora *</label><span class="text-[10px] text-gray-400">{{ nameLength }}/100</span></div><input v-model="form.name" type="text" required maxlength="100" class="w-full px-3 py-2 text-xs rounded-xl border-2 border-gray-200 focus:border-emerald-500 focus:ring-4 focus:ring-emerald-500/10 transition-all"></div>
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                            <div><label class="block text-[11px] font-medium text-gray-700 mb-1">Nama Ilmiah</label><input v-model="form.scientific_name" type="text" class="w-full px-3 py-2 text-xs rounded-xl border-2 border-gray-200 focus:border-emerald-500 italic"></div>
                            <div><label class="block text-[11px] font-medium text-gray-700 mb-1">Nama Lokal</label><input v-model="form.local_name" type="text" class="w-full px-3 py-2 text-xs rounded-xl border-2 border-gray-200 focus:border-emerald-500"></div>
                        </div>
                    </div>
                </div>
                ...
                <!-- Description -->
                <div class="form-section bg-white rounded-2xl shadow-lg border border-gray-100 p-5">
                    <div class="flex items-center justify-between mb-4"><h2 class="text-sm font-bold text-gray-900 flex items-center gap-2"><FileText class="w-4 h-4 text-purple-600" />Deskripsi</h2><span class="text-[10px] text-gray-400">{{ descLength }} karakter</span></div>
                    <TinyMceEditor v-model="form.description" name="description" id="flora-edit-desc" :height="350" placeholder="Deskripsi flora..." />
                </div>

                <!-- SEO -->
                <div class="form-section bg-white rounded-2xl shadow-lg border border-gray-100 p-5">
                    <div class="flex items-center justify-between mb-4"><h2 class="text-sm font-bold text-gray-900 flex items-center gap-2"><Sparkles class="w-4 h-4 text-blue-600" />SEO</h2><button type="button" @click="generateAI('seo')" class="px-2 py-1 rounded-lg bg-blue-100 text-blue-600 text-[9px] font-bold hover:bg-blue-200 flex items-center gap-1"><Wand2 class="w-3 h-3" /> Generate</button></div>
                    <div class="space-y-3">
                        <div><div class="flex justify-between mb-1"><label class="text-[10px] font-medium text-gray-700">Meta Title</label><span class="text-[9px] text-gray-400">{{ form.meta_title?.length || 0 }}/60</span></div><input v-model="form.meta_title" type="text" maxlength="60" class="w-full px-3 py-2 text-xs rounded-xl border-2 border-gray-200 focus:border-blue-500"></div>
                        <div><div class="flex justify-between mb-1"><label class="text-[10px] font-medium text-gray-700">Meta Description</label><span class="text-[9px] text-gray-400">{{ form.meta_description?.length || 0 }}/160</span></div><textarea v-model="form.meta_description" rows="2" maxlength="160" class="w-full px-3 py-2 text-xs rounded-xl border-2 border-gray-200 focus:border-blue-500 resize-none"></textarea></div>
                    </div>
                </div>
            </div>
            ...
            <div class="space-y-4">
                ...
                <div class="form-section bg-white rounded-2xl shadow-lg border-l-4 border-teal-500 p-5">
                    ...
                    <div v-if="flora.image_url && !imagePreview" class="mb-3">
                        <div class="relative aspect-video rounded-xl overflow-hidden shadow-lg group">
                            <img :src="flora.image_url" class="w-full h-full object-cover">
                    ...
                    <div v-else-if="imagePreview" class="relative rounded-xl overflow-hidden shadow-lg group">
                        ...
                        <span class="absolute top-2 left-2 px-2 py-0.5 bg-emerald-500 text-white text-[9px] rounded-lg font-bold">Cover Baru</span>
                        <div class="absolute inset-0 bg-black/40 flex items-center justify-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
