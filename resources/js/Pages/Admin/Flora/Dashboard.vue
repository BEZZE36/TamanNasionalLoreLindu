...
import { Flower2, Eye, Heart, MessageCircle, Star, TrendingUp, BarChart3, Sparkles, Shield, Leaf } from 'lucide-vue-next';
...
const props = defineProps({
    ...
    topFlora: { type: Array, default: () => [] },
    recentFlora: { type: Array, default: () => [] },
...
const changePeriod = () => router.get('/admin/flora/dashboard', { period: period.value }, { preserveState: true });
        ...
        <div class="relative overflow-hidden rounded-2xl bg-gradient-to-r from-green-500 via-emerald-500 to-teal-500 p-6 shadow-2xl">
            ...
            <div class="header-content relative flex flex-col lg:flex-row items-start lg:items-center justify-between gap-4">
                <div class="flex items-center gap-4">
                    ...
                    <div>
                        <div class="flex items-center gap-2 mb-1">
                            <h1 class="text-xl font-bold text-white tracking-tight drop-shadow-lg">Dashboard Flora</h1>
                        ...
                        <p class="text-emerald-100/80 text-xs">Statistik dan analitik data flora</p>
                    </div>
                </div>
                ...
                <div class="flex items-center gap-3">
                    ...
                    <Link href="/admin/flora" class="group inline-flex items-center gap-2 px-5 py-2.5 rounded-xl bg-white text-emerald-600 text-xs font-bold hover:bg-emerald-50 hover:shadow-xl hover:-translate-y-0.5 transition-all duration-300 shadow-lg">
                        <Flower2 class="w-4 h-4 group-hover:scale-110 transition-transform" />
            ...
            <div class="stat-card group relative overflow-hidden rounded-xl bg-white p-4 shadow-lg border border-gray-100 hover:shadow-xl hover:-translate-y-1 transition-all duration-300">
                ...
                <div class="relative flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p class="text-[10px] text-gray-500 font-medium">Total Flora</p>
                    </div>
                </div>
            </div>
        ...
        <div class="grid lg:grid-cols-2 gap-5">
            ...
            <div class="chart-section rounded-xl bg-white p-5 shadow-lg border border-gray-100">
                <h3 class="text-sm font-bold text-gray-900 mb-4 flex items-center gap-2">
                    ...
                    Top 10 Flora Populer
                </h3>
                <div class="space-y-2.5 max-h-48 overflow-y-auto">
                    <div v-for="(flora, i) in topFlora" :key="flora.id" class="flex items-center gap-3 group p-2 rounded-lg hover:bg-gray-50 transition-colors">
                        ...
                        <div class="flex-1 min-w-0">
                            <Link :href="`/admin/flora/${flora.id}/edit`" class="text-xs font-medium text-gray-900 group-hover:text-emerald-600 truncate block transition-colors">
                                {{ flora.name }}
                            </Link>
                            <p class="text-[10px] text-gray-400">{{ formatNumber(flora.views_count || 0) }} views • {{ flora.category }}</p>
                        </div>
                    </div>
                    <p v-if="!topFlora?.length" class="text-gray-400 text-xs text-center py-4">Belum ada data</p>
                </div>
            </div>
        </div>
        ...
        <div class="bottom-section rounded-xl bg-white p-5 shadow-lg border border-gray-100">
            <h3 class="text-sm font-bold text-gray-900 mb-4 flex items-center gap-2">
                <div class="flex h-8 w-8 items-center justify-center rounded-lg bg-gradient-to-br from-blue-500 to-cyan-600 shadow-md">
                    <Flower2 class="w-4 h-4 text-white" />
                </div>
                Flora Terbaru
            </h3>
            <div class="grid grid-cols-2 lg:grid-cols-4 gap-3">
                <div v-for="flora in recentFlora" :key="flora.id" class="group rounded-xl overflow-hidden border border-gray-100 hover:shadow-lg transition-all">
                    <div class="aspect-video bg-gray-100 relative">
                        <img v-if="flora.image_url" :src="flora.image_url" class="w-full h-full object-cover" />
                        <div class="absolute top-2 right-2">
                            <span :class="['px-1.5 py-0.5 rounded text-[8px] font-bold', flora.category === 'endemik' ? 'bg-purple-500 text-white' : flora.category === 'langka' ? 'bg-red-500 text-white' : 'bg-gray-500 text-white']">
                                {{ flora.category }}
                            </span>
                        </div>
                    </div>
                    <div class="p-3">
                        <Link :href="`/admin/flora/${flora.id}/edit`" class="text-xs font-bold text-gray-900 group-hover:text-emerald-600 truncate block">{{ flora.name }}</Link>
                        <p class="text-[10px] text-gray-400 mt-0.5 italic">{{ flora.scientific_name }}</p>
                    </div>
                </div>
                <p v-if="!recentFlora?.length" class="col-span-4 text-gray-400 text-xs text-center py-8">Belum ada flora</p>
            </div>
        </div>
    </div>
</template>
