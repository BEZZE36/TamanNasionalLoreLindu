...
import { MessageCircle, Plus, Search, Trash2, Eye, EyeOff, Pin, Reply, ArrowLeft, Filter, Sparkles, AlertTriangle, Loader2, Image } from 'lucide-vue-next';
...
onMounted(() => {
    ctx = gsap.context(() => {
        ...
        gsap.fromTo('.comment-item',
            { opacity: 0, x: -20 },
            { opacity: 1, x: 0, duration: 0.4, stagger: 0.05, delay: 0.4, ease: 'power2.out' }
        );
    });
});
...
const openReplyModal = (commentId, galleryId, username) => {
    replyToId.value = commentId;
    newComment.value = { gallery_id: galleryId, content: `@${username || 'user'} ` };
    showAddModal.value = true;
};
...
const toggleReplyStatus = async (replyId) => {
    try {
        await fetch(`/admin/gallery/comments/${replyId}/toggle-status`, {
            method: 'POST',
            headers: { 'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content }
        });
        router.reload();
...
const deleteReply = async (replyId) => {
    ...
    try {
        await fetch(`/admin/gallery/comments/${replyId}`, {
            method: 'DELETE',
            headers: { 'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content }
        });
        router.reload();
    ...
    <div class="min-h-screen space-y-5">
                ...
                <div class="flex items-center gap-4">
                    ...
                    <div>
                        <div class="flex items-center gap-2 mb-1">
                            <h1 class="text-xl font-bold text-white tracking-tight drop-shadow-lg">Komentar Gallery</h1>
                            <span class="px-2 py-0.5 bg-white/20 rounded-full text-[10px] font-bold text-white/90 backdrop-blur-sm">
                                {{ stats.total }} Total
                            </span>
                        </div>
                        <p class="text-purple-100/80 text-xs">Kelola dan moderasi komentar galeri</p>
                    </div>
                </div>
                
                <div class="flex items-center gap-3">
                    <Link href="/admin/gallery" class="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl bg-white/10 text-white text-xs font-semibold backdrop-blur-sm border border-white/20 hover:bg-white/20 transition-all">
                        <ArrowLeft class="w-4 h-4" /> Kembali
                    </Link>
                    <button v-if="selectedIds.length" @click="promptDeleteSelected"
                        class="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl bg-red-500/80 text-white text-xs font-semibold backdrop-blur-sm hover:bg-red-600 transition-all">
                        <Trash2 class="w-4 h-4" /> Hapus ({{ selectedIds.length }})
                    </button>
                    <button @click="openAddModal"
                        class="group inline-flex items-center gap-2 px-5 py-2.5 rounded-xl bg-white text-violet-600 text-xs font-bold hover:bg-violet-50 hover:shadow-xl hover:-translate-y-0.5 transition-all duration-300 shadow-lg">
        ...
        <div class="grid grid-cols-2 lg:grid-cols-4 gap-3">
            <div class="stat-card group relative overflow-hidden rounded-xl p-4 shadow-lg hover:shadow-xl hover:-translate-y-1 transition-all duration-300" style="background: linear-gradient(135deg, #8b5cf6 0%, #a855f7 100%);">
                ...
                <p class="text-[10px] font-medium" style="color: rgba(255,255,255,0.8);">Total Komentar</p>
            </div>
        ...
        <div class="rounded-xl bg-white p-4 shadow-lg border border-gray-100">
            <div class="flex flex-col lg:flex-row gap-3">
                ...
                <select v-model="filterGallery" @change="search"
                    class="px-4 py-2.5 text-xs rounded-xl border-2 border-gray-200 bg-gray-50/50 focus:border-violet-500 focus:ring-4 focus:ring-violet-500/10 transition-all cursor-pointer">
                    <option value="">Semua Album</option>
                    <option v-for="g in galleryList" :key="g.id" :value="g.id">{{ g.title }}</option>
                </select>
                <button @click="search" class="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl bg-gradient-to-r from-violet-500 to-purple-600 text-white text-xs font-semibold hover:shadow-lg transition-all">
        ...
        <div class="rounded-xl bg-white shadow-lg border border-gray-100 overflow-hidden">
            <div v-if="comments?.data?.length" class="divide-y divide-gray-100">
                <div v-for="comment in comments.data" :key="comment.id"
                    :class="['comment-item p-4 hover:bg-gradient-to-r hover:from-violet-50/50 hover:to-purple-50/30 transition-all', comment.is_pinned ? 'bg-violet-50/30' : '']">
                    <div class="flex items-start gap-3">
                        <input type="checkbox" :value="comment.id" v-model="selectedIds"
                            class="mt-1 w-4 h-4 rounded border-gray-300 text-violet-500 focus:ring-violet-500">
                        
                        <div class="w-10 h-10 rounded-full bg-gradient-to-br from-violet-500 to-purple-600 flex items-center justify-center text-white font-bold text-xs flex-shrink-0 shadow-md">
                        ...
                        <div class="flex-1 min-w-0">
                            <div class="flex items-center gap-2 flex-wrap">
                                ...
                                <span v-if="comment.is_pinned" class="inline-flex items-center gap-1 px-2 py-0.5 bg-gradient-to-r from-violet-500 to-purple-600 text-white text-[10px] font-bold rounded-full shadow-sm">
                            ...
                            <div v-if="comment.replies?.length" class="mt-4 space-y-3 pl-4 border-l-2 border-violet-200">
                                <div v-for="reply in comment.replies" :key="reply.id" class="flex items-start gap-3 group/reply p-2 rounded-lg hover:bg-gray-50 transition-all">
                                    ...
                                    <div class="flex-1 min-w-0">
                                        <div class="flex items-center gap-2">
                                            ...
                                            <span v-if="reply.is_approved === false" class="px-1.5 py-0.5 bg-amber-100 text-amber-600 text-[9px] font-bold rounded-full">Tersembunyi</span>
                                        </div>
                                    ...
                                    <div class="flex items-center gap-0.5 opacity-0 group-hover/reply:opacity-100 transition-opacity flex-shrink-0">
                                        <button @click="openReplyModal(comment.id, comment.gallery_id, reply.user?.name || reply.admin?.name || 'user')"
                                        ...
                                        <button @click="toggleReplyStatus(reply.id)" 
                                            :class="reply.is_approved ? 'text-emerald-500 hover:text-amber-600 hover:bg-amber-50' : 'text-amber-500 hover:text-emerald-600 hover:bg-emerald-50'"
                                            class="p-1.5 rounded-lg transition-all" 
                                            :title="reply.is_approved ? 'Sembunyikan' : 'Tampilkan'">
                                            <component :is="reply.is_approved ? EyeOff : Eye" class="w-3 h-3" />
                                        </button>
                        ...
                        <div class="flex items-center gap-1 flex-shrink-0">
                            <button @click="openReplyModal(comment.id, comment.gallery_id, comment.user?.name || comment.admin?.name || 'user')" 
                            ...
                            <button @click="toggleStatus(comment.id)"
                                ...
                                <component :is="comment.is_visible ? EyeOff : Eye" class="w-4 h-4" />
                            </button>
                            <button @click="togglePin(comment.id)"
                                :class="comment.is_pinned ? 'text-violet-500 bg-violet-50' : 'text-gray-400 hover:text-violet-600 hover:bg-violet-50'"
            ...
            <div v-else class="text-center py-16">
                <div class="w-16 h-16 bg-gradient-to-br from-violet-100 to-purple-100 rounded-2xl flex items-center justify-center mx-auto mb-4">
                    <MessageCircle class="w-8 h-8 text-violet-400" />
                </div>
            ...
            <div v-if="comments?.links" class="px-5 py-4 border-t border-gray-100 flex justify-center gap-1 bg-gray-50/50">
                <Link v-for="link in comments.links" :key="link.label" :href="link.url || '#'"
                    :class="[
                        ...
                        link.active ? 'bg-gradient-to-r from-violet-500 to-purple-600 text-white shadow-md' : 'text-gray-600 hover:bg-gray-100',
                ...
                <div v-if="showDeleteModal" class="fixed inset-0 z-50 flex items-center justify-center p-4">
                    ...
                    <div class="relative bg-white rounded-2xl shadow-2xl max-w-md w-full p-6">
                        ...
                        <div class="flex gap-3 justify-end">
                            ...
                            <button @click="confirmDelete" class="px-5 py-2.5 rounded-xl bg-gradient-to-r from-red-500 to-rose-600 text-white text-xs font-semibold hover:shadow-lg transition-all flex items-center gap-2">
                ...
                <div v-if="showAddModal" class="fixed inset-0 z-50 flex items-center justify-center p-4">
                    ...
                    <div class="relative bg-white rounded-2xl shadow-2xl max-w-lg w-full p-6">
                        <h3 class="text-lg font-bold text-gray-900 mb-4 flex items-center gap-2">
                            <MessageCircle class="w-5 h-5 text-violet-600" />
                            {{ replyToId ? 'Balas Komentar' : 'Tambah Komentar' }}
                        </h3>
                        
                        <div v-if="!replyToId" class="mb-4">
                            <label class="block text-xs font-medium text-gray-700 mb-2">Album</label>
                            <select v-model="newComment.gallery_id" class="w-full px-4 py-2.5 text-xs rounded-xl border-2 border-gray-200 focus:border-violet-500 focus:ring-4 focus:ring-violet-500/10">
                        ...
                        <div class="mb-6">
                            <label class="block text-xs font-medium text-gray-700 mb-2">Komentar</label>
                            <textarea v-model="newComment.content" rows="4" placeholder="Tulis komentar..."
                                class="w-full px-4 py-3 text-xs rounded-xl border-2 border-gray-200 focus:border-violet-500 focus:ring-4 focus:ring-violet-500/10 resize-none"></textarea>
                        </div>
                        
                        <div class="flex gap-3 justify-end">
                            <button @click="showAddModal = false" class="px-4 py-2.5 rounded-xl bg-gray-100 text-gray-700 text-xs font-semibold hover:bg-gray-200 transition-all">Batal</button>
                            <button @click="submitComment" :disabled="isSubmitting"
                                class="px-5 py-2.5 rounded-xl bg-gradient-to-r from-violet-500 to-purple-600 text-white text-xs font-semibold hover:shadow-lg transition-all disabled:opacity-50 flex items-center gap-2">
