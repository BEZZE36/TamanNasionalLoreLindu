...
import { Image, Save, ArrowLeft, Upload, X, Plus, Video, Calendar, MapPin, Tag, Info, Search } from 'lucide-vue-next';
import { ref } from 'vue';
...
const submit = () => {
    form.transform(data => {
        ...
        return fd;
    }).post('/admin/gallery', { forceFormData: true, onSuccess: () => router.visit('/admin/gallery') });
};
</script>
        ...
        <div class="flex items-center gap-3">
            ...
            <div>
                <h1 class="text-lg font-bold text-gray-900 flex items-center gap-2">
                    <Image class="w-7 h-7 text-violet-600" />Tambah Album Galeri
                </h1>
                <p class="text-gray-500 text-sm">Buat album baru untuk galeri TNLL</p>
            </div>
        </div>
            ...
            <div class="lg:col-span-2 space-y-6">
                ...
                <div class="bg-white rounded-2xl shadow-lg border border-gray-100 p-6">
                    <h2 class="text-lg font-bold text-gray-900 mb-4 flex items-center gap-2">
                        <Image class="w-5 h-5 text-emerald-600" />Cover Album *
                    </h2>
                    <label class="block cursor-pointer">
                        ...
                        <div v-else class="border-2 border-dashed border-gray-300 hover:border-violet-400 rounded-xl p-8 transition-all text-center">
                            ...
                            <p class="text-sm text-gray-600 font-medium">Klik untuk upload Cover</p>
                            <p class="text-xs text-gray-400">JPG, PNG, WEBP (Max 5MB)</p>
                        </div>
                        <input type="file" accept="image/*" @change="handleCoverUpload" class="hidden">
                    </label>
                    <p v-if="form.errors.cover_image" class="text-red-500 text-xs mt-2">{{ form.errors.cover_image }}</p>
                </div>
                ...
                <div class="bg-white rounded-2xl shadow-lg border border-gray-100 p-6">
                    <div class="flex items-center justify-between mb-4">
                        <h2 class="text-lg font-bold text-gray-900 flex items-center gap-2">
                            <Plus class="w-5 h-5 text-blue-600" />Foto Galeri
                        </h2>
                        <span :class="['px-3 py-1 rounded-full text-sm font-bold', galleryPreviews.length > 0 ? 'bg-violet-100 text-violet-700' : 'bg-gray-100 text-gray-500']">
                            {{ galleryPreviews.length }} foto
                        </span>
                    </div>
                    <div @click="$refs.galleryInput.click()" @dragover.prevent="isDragging = true" @dragleave.prevent="isDragging = false" @drop.prevent="handleDrop"
                        :class="['border-2 border-dashed rounded-xl p-8 text-center transition-all cursor-pointer', isDragging ? 'border-violet-500 bg-violet-50' : 'border-gray-300 hover:border-violet-400']">
                        <Upload :class="['w-10 h-10 mx-auto mb-3', isDragging ? 'text-violet-500' : 'text-gray-400']" />
                        <p class="text-gray-600 font-medium">{{ isDragging ? 'Lepas file di sini!' : 'Drag & drop foto di sini' }}</p>
            ...
            <div class="space-y-4">
                ...
                <div class="space-y-3">
                    <button type="submit" :disabled="form.processing"
                        ...
                        <Save class="w-5 h-5" />{{ form.processing ? 'Menyimpan...' : 'Simpan Album' }}
                    </button>
