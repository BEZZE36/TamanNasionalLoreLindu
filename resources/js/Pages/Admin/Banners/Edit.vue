...
const props = defineProps({ banner: { type: Object, required: true } });
...
const form = useForm({
    ...
    start_at: props.banner.start_at?.split('T')[0] || '',
    end_at: props.banner.end_at?.split('T')[0] || '',
    is_active: props.banner.is_active ?? true,
    _method: 'PUT'
});

const desktopPreview = ref(props.banner.image_url);
const mobilePreview = ref(props.banner.mobile_image_url);
...
const submit = () => { form.post(`/admin/banners/${props.banner.id}`); };
</script>
    ...
    <div class="max-w-4xl mx-auto space-y-6">
        ...
        <div class="relative overflow-hidden rounded-2xl bg-gradient-to-r from-orange-500 via-red-500 to-pink-500 p-5 shadow-xl">
            ...
            <div class="relative flex items-center gap-3">
                ...
                <div>
                    <h1 class="text-3xl font-extrabold text-white tracking-tight drop-shadow-lg">Edit Banner</h1>
                    <p class="mt-1 text-orange-100/90">{{ banner.title }}</p>
                </div>
            </div>
        </div>
        ...
        <form @submit.prevent="submit" class="space-y-4">
            ...
            <div class="bg-white rounded-2xl shadow-xl border border-gray-100 overflow-hidden">
                ...
                <div class="p-8 grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-2">Desktop (1920x600)</label>
                    ...
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-2">Mobile (720x480)</label>
                ...
                <div class="p-8 space-y-6">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                        <div>
                            ...
                            <input v-model="form.link_url" type="text" class="w-full px-4 py-3 rounded-xl border border-gray-200 focus:border-green-500 focus:ring-2 focus:ring-green-500/20">
                        </div>
                        <div>
            ...
            <div class="flex justify-end">
                <button type="submit" :disabled="form.processing"
                    ...
                    <Save class="w-6 h-6" /><span>{{ form.processing ? 'Menyimpan...' : 'Perbarui Banner' }}</span>
                </button>
            </div>
        </form>
    </div>
