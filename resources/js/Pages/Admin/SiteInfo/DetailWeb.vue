...
import { Globe, Phone, Mail, MapPin, Clock, Save, CheckCircle, Loader2 } from 'lucide-vue-next';
...
<template>
    <div class="min-h-screen space-y-8">
        <!-- Header -->
        <div class="relative overflow-hidden rounded-2xl bg-gradient-to-r from-violet-600 via-purple-600 to-indigo-600 p-5 shadow-xl">
            ...
            <div class="relative flex items-center gap-3">
                <div class="flex h-9 w-9 items-center justify-center rounded-lg bg-white/20 backdrop-blur-xl ring-2 ring-white/30 shadow-xl p-2.5">
                    <Globe class="h-5 w-5 text-white" />
                </div>
                <div>
                    <h1 class="text-xl font-bold text-white tracking-tight drop-shadow-lg">Detail Web</h1>
                    <p class="mt-1.5 text-purple-100/90 text-lg font-medium">Kelola kontak dan sosial media website</p>
                </div>
            </div>
        </div>
        ...
        <Transition name="slide">
            <div v-if="flash.success" class="flex items-center gap-4 rounded-2xl bg-gradient-to-r from-emerald-50 to-teal-50 border border-emerald-200/60 px-6 py-5 shadow-lg">
                ...
                <div>
                    ...
                    <p class="text-emerald-600 text-sm">{{ flash.success }}</p>
                </div>
            </div>
        </Transition>
        ...
        <form @submit.prevent="submit" class="space-y-5">
            <!-- Contact Information -->
            <div class="rounded-3xl bg-white shadow-xl border border-gray-100 overflow-hidden">
                <div class="relative px-8 py-6 bg-gradient-to-r from-blue-50 via-indigo-50 to-purple-50 border-b border-gray-100">
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p class="text-gray-500 text-sm mt-0.5">Atur informasi kontak yang tampil di website</p>
                        </div>
                    </div>
                </div>
                ...
                <div class="p-8 space-y-8">
                        ...
                        <div>
                            ...
                            <div class="relative">
                                ...
                                <input type="tel" v-model="form.contact_phone" placeholder="877 5169 0646"
                                    class="w-full rounded-xl border-2 border-gray-200 bg-gray-50/50 pl-14 pr-5 py-4 focus:border-green-500 focus:ring-4 focus:ring-green-500/10 focus:bg-white transition-all">
                            </div>
                        </div>
                        <div>
                            <label class="flex items-center gap-2 text-sm font-semibold text-gray-700 mb-3">
                            ...
                            <div class="relative">
                                ...
                                <input type="tel" v-model="form.contact_whatsapp" placeholder="812 345 6789"
                                    class="w-full rounded-xl border-2 border-gray-200 bg-gray-50/50 pl-14 pr-5 py-4 focus:border-emerald-500 focus:ring-4 focus:ring-emerald-500/10 focus:bg-white transition-all">
                            </div>
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <input type="email" v-model="form.contact_email" placeholder="info@lorelindu.info"
                            class="w-full rounded-xl border-2 border-gray-200 bg-gray-50/50 px-5 py-4 focus:border-blue-500 focus:ring-4 focus:ring-blue-500/10 focus:bg-white transition-all">
                    </div>

                    <!-- Divider -->
                    <div class="relative">
                        <div class="absolute inset-0 flex items-center"><div class="w-full border-t border-gray-200"></div></div>
                        <div class="relative flex justify-center"><span class="bg-white px-4 text-sm text-gray-500 font-medium">Lokasi & Alamat</span></div>
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                        <div>
                            ...
                            <input type="text" v-model="form.contact_latitude" placeholder="-0.8924096"
                                class="w-full rounded-xl border-2 border-gray-200 bg-gray-50/50 px-5 py-4 font-mono focus:border-rose-500 focus:ring-4 focus:ring-rose-500/10 focus:bg-white transition-all">
                        </div>
                        <div>
                            <label class="flex items-center gap-2 text-sm font-semibold text-gray-700 mb-3">
                                <span class="flex h-8 w-8 items-center justify-center rounded-lg bg-rose-100 text-rose-600">
                    ...
                    <button type="button" @click="getAddress" :disabled="loadingAddress"
                        class="inline-flex items-center gap-3 px-6 py-3.5 rounded-xl bg-gradient-to-r from-blue-500 to-indigo-600 text-white font-semibold shadow-lg shadow-blue-500/30 hover:shadow-xl hover:-translate-y-0.5 transition-all disabled:opacity-60">
                        <Loader2 v-if="loadingAddress" class="w-5 h-5 animate-spin" />
                        <MapPin v-else class="w-5 h-5" />
                        {{ loadingAddress ? 'Mencari Alamat...' : 'Dapatkan Alamat dari Koordinat' }}
                    </button>
                    ...
                    <div>
                        ...
                        <textarea v-model="form.contact_address" rows="3" placeholder="Jl. Prof. Dr. Yamin No. 17, Palu, Sulawesi Tengah"
                            class="w-full rounded-xl border-2 border-gray-200 bg-gray-50/50 px-5 py-4 focus:border-amber-500 focus:ring-4 focus:ring-amber-500/10 focus:bg-white transition-all resize-none"></textarea>
                    </div>
                    ...
                    <div>
                        ...
                        <div class="space-y-3">
                            <div v-for="day in days" :key="day.key"
                                class="flex items-center gap-4 p-4 rounded-xl bg-gray-50/80 border-2 border-gray-100 hover:border-violet-200 transition-all">
                                <div class="w-24 font-semibold text-gray-700">{{ day.name }}</div>
                                
                                <label class="relative inline-flex items-center cursor-pointer">
                                    <input type="checkbox" v-model="form[`hours_${day.key}_open`]" class="sr-only peer">
                                    <div class="w-14 h-7 bg-gray-300 peer-focus:ring-4 peer-focus:ring-violet-500/20 rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-0.5 after:start-[4px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-6 after:w-6 after:transition-all peer-checked:bg-gradient-to-r peer-checked:from-violet-500 peer-checked:to-purple-600"></div>
                                </label>
                                ...
                                <div v-if="form[`hours_${day.key}_open`]" class="flex items-center gap-2 flex-1 justify-end">
                                    <input type="time" v-model="form[`hours_${day.key}_start`]"
                                        class="rounded-lg border-2 border-gray-200 bg-white px-3 py-2 text-sm focus:border-violet-500 focus:ring-4 focus:ring-violet-500/10 transition-all">
                                    <span class="text-gray-400 font-medium">-</span>
                                    <input type="time" v-model="form[`hours_${day.key}_end`]"
                                        class="rounded-lg border-2 border-gray-200 bg-white px-3 py-2 text-sm focus:border-violet-500 focus:ring-4 focus:ring-violet-500/10 transition-all">
                                </div>
                                <div v-else class="flex-1 text-right">
                                    <span class="text-gray-400 text-sm italic">Tidak beroperasi</span>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="relative">
                        <div class="absolute inset-0 flex items-center"><div class="w-full border-t border-gray-200"></div></div>
                        <div class="relative flex justify-center"><span class="bg-white px-4 text-sm text-gray-500 font-medium">Embed Peta</span></div>
                    </div>
                    ...
                    <div>
                        ...
                        <textarea v-model="form.google_maps_embed" rows="4" placeholder='<iframe src="https://www.google.com/maps/embed?..." ...></iframe>'
                            class="w-full rounded-xl border-2 border-gray-200 bg-gray-50/50 px-5 py-4 font-mono text-xs focus:border-red-500 focus:ring-4 focus:ring-red-500/10 focus:bg-white transition-all resize-none"></textarea>
                    </div>

                    <!-- Map Preview -->
                    <Transition name="fade">
                        <div v-if="showMapPreview" class="rounded-2xl overflow-hidden border-2 border-gray-200 shadow-lg">
            ...
            <!-- Social Media -->
            <div class="rounded-3xl bg-white shadow-xl border border-gray-100 overflow-hidden">
                <div class="relative px-8 py-6 bg-gradient-to-r from-pink-50 via-rose-50 to-orange-50 border-b border-gray-100">
                    <div class="flex items-center gap-3">
                        <div class="flex h-14 w-14 items-center justify-center rounded-2xl bg-gradient-to-br from-pink-500 to-rose-600 shadow-lg shadow-pink-500/30">
                            <svg class="w-7 h-7 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 8h2a2 2 0 012 2v6a2 2 0 01-2 2h-2v4l-4-4H9a1.994 1.994 0 01-1.414-.586m0 0L11 14h4a2 2 0 002-2V6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2v4l.586-.586z"/></svg>
                        </div>
                        <div>
                            <h2 class="text-xl font-bold text-gray-900">Media Sosial</h2>
                            <p class="text-gray-500 text-sm mt-0.5">Hubungkan dengan akun media sosial</p>
                        </div>
                    </div>
                </div>
                ...
                <div class="p-8 space-y-6">
                    <!-- Facebook -->
                    <div class="flex items-center gap-4 p-4 rounded-2xl bg-gray-50/50 border-2 border-transparent hover:border-[#1877F2]/20 hover:bg-blue-50/30 transition-all group">
                        <div class="flex h-14 w-14 items-center justify-center rounded-2xl bg-[#1877F2] shadow-lg shadow-[#1877F2]/30 group-hover:scale-110 transition-transform">
                        ...
                        <div class="flex-1">
                            <label class="text-sm font-bold text-gray-800 block mb-2">Facebook</label>
                            <input type="url" v-model="form.facebook_url" placeholder="https://facebook.com/yourpage"
                                class="w-full rounded-xl border-2 border-gray-200 bg-white px-4 py-3 focus:border-[#1877F2] focus:ring-4 focus:ring-[#1877F2]/10 transition-all">
                        </div>
                    </div>

                    <!-- Instagram -->
                    <div class="flex items-center gap-4 p-4 rounded-2xl bg-gray-50/50 border-2 border-transparent hover:border-[#dc2743]/20 hover:bg-pink-50/30 transition-all group">
                        <div class="flex h-14 w-14 items-center justify-center rounded-2xl bg-gradient-to-br from-[#f09433] via-[#dc2743] to-[#bc1888] shadow-lg shadow-[#dc2743]/30 group-hover:scale-110 transition-transform">
                        ...
                        <div class="flex-1">
                            <label class="text-sm font-bold text-gray-800 block mb-2">Instagram</label>
                            <input type="url" v-model="form.instagram_url" placeholder="https://instagram.com/yourprofile"
                                class="w-full rounded-xl border-2 border-gray-200 bg-white px-4 py-3 focus:border-[#dc2743] focus:ring-4 focus:ring-[#dc2743]/10 transition-all">
                        </div>
                    </div>

                    <!-- YouTube -->
                    <div class="flex items-center gap-4 p-4 rounded-2xl bg-gray-50/50 border-2 border-transparent hover:border-[#FF0000]/20 hover:bg-red-50/30 transition-all group">
                        <div class="flex h-14 w-14 items-center justify-center rounded-2xl bg-[#FF0000] shadow-lg shadow-[#FF0000]/30 group-hover:scale-110 transition-transform">
                            <svg class="w-7 h-7 text-white" fill="currentColor" viewBox="0 0 24 24"><path d="M23.498 6.186a3.016 3.016 0 00-2.122-2.136C19.505 3.545 12 3.545 12 3.545s-7.505 0-9.377.505A3.017 3.017 0 00.502 6.186C0 8.07 0 12 0 12s0 3.93.502 5.814a3.016 3.016 0 002.122 2.136c1.871.505 9.376.505 9.376.505s7.505 0 9.377-.505a3.015 3.015 0 002.122-2.136C24 15.93 24 12 24 12s0-3.93-.502-5.814zM9.545 15.568V8.432L15.818 12l-6.273 3.568z"/></svg>
                        </div>
                        <div class="flex-1">
                            <label class="text-sm font-bold text-gray-800 block mb-2">YouTube</label>
                            <input type="url" v-model="form.youtube_url" placeholder="https://youtube.com/@yourchannel"
                                class="w-full rounded-xl border-2 border-gray-200 bg-white px-4 py-3 focus:border-[#FF0000] focus:ring-4 focus:ring-[#FF0000]/10 transition-all">
                        </div>
                    </div>

                    <!-- TikTok -->
                    <div class="flex items-center gap-4 p-4 rounded-2xl bg-gray-50/50 border-2 border-transparent hover:border-gray-400/20 hover:bg-gray-100/50 transition-all group">
                        <div class="flex h-14 w-14 items-center justify-center rounded-2xl bg-black shadow-lg shadow-black/30 group-hover:scale-110 transition-transform">
                        ...
                        <div class="flex-1">
                            <label class="text-sm font-bold text-gray-800 block mb-2">TikTok</label>
                            <input type="url" v-model="form.tiktok_url" placeholder="https://tiktok.com/@yourprofile"
                                class="w-full rounded-xl border-2 border-gray-200 bg-white px-4 py-3 focus:border-gray-800 focus:ring-4 focus:ring-gray-800/10 transition-all">
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="flex justify-end pt-4">
                <button type="submit" :disabled="form.processing"
                    class="group relative inline-flex items-center gap-3 rounded-2xl bg-gradient-to-r from-violet-600 via-purple-600 to-indigo-600 px-10 py-5 text-white font-bold text-lg shadow-2xl shadow-purple-500/30 hover:shadow-purple-500/50 hover:-translate-y-1 transition-all disabled:opacity-50">
                    <Save class="w-6 h-6" />
