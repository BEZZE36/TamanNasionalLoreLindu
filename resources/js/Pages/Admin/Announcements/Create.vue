...
import { Bell, ArrowLeft, Save, Type, MessageSquare, Palette, Link as LinkIcon, Eye } from 'lucide-vue-next';

defineOptions({ layout: AdminLayout });

const form = useForm({
    title: '', message: '', type: 'banner', notification_type: 'info',
    bg_color: '#fbbf24', text_color: '#ffffff', link_url: '', link_text: '',
    is_dismissible: true, is_active: false, start_date: '', end_date: ''
});

const submit = () => form.post('/admin/announcements', { onSuccess: () => router.visit('/admin/announcements') });
</script>
    ...
    <div class="max-w-3xl mx-auto space-y-6">
        ...
        <div class="flex items-center gap-3">
            ...
            <div>
                <h1 class="text-lg font-bold text-gray-900 flex items-center gap-2">
                    <Bell class="w-7 h-7 text-amber-600" />Buat Pengumuman Baru
                </h1>
            </div>
        </div>
        ...
        <form @submit.prevent="submit" class="space-y-4">
            ...
            <button type="submit" :disabled="form.processing"
                class="w-full py-4 bg-gradient-to-r from-amber-500 to-yellow-500 text-white font-bold text-lg rounded-xl shadow-lg shadow-amber-500/30 hover:shadow-amber-500/50 transition-all flex items-center justify-center gap-2 disabled:opacity-50">
                <Save class="w-5 h-5" />{{ form.processing ? 'Menyimpan...' : 'Simpan Pengumuman' }}
            </button>
        </form>
    </div>
</template>
