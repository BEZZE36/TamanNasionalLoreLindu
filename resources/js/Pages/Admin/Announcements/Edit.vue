...
const form = useForm({
    ...
    type: props.announcement.type || 'banner',
    notification_type: props.announcement.notification_type || 'info',
    bg_color: props.announcement.bg_color || '#f59e0b',
    ...
    is_dismissible: props.announcement.is_dismissible ?? true,
    is_active: props.announcement.is_active || false,
    priority: props.announcement.priority || 1,
    start_at: parseDate(props.announcement.start_at),
    end_at: parseDate(props.announcement.end_at),
    // Advanced features
    target_audience: props.announcement.target_audience || 'all',
    target_pages: props.announcement.target_pages || [],
    exclude_pages: props.announcement.exclude_pages || [],
    show_days: props.announcement.show_days || [],
    show_time_start: props.announcement.show_time_start || '',
    show_time_end: props.announcement.show_time_end || '',
    // Countdown
    show_countdown: props.announcement.show_countdown || false,
    countdown_label: props.announcement.countdown_label || '',
    // Extra buttons
    extra_buttons: props.announcement.extra_buttons || []
});
...
const submit = () => {
    ...
    form.transform((data) => ({
        ...data,
        _method: 'PUT'
    })).post(`/admin/announcements/${props.announcement.id}`, {
        forceFormData: true,
                ...
                <div class="flex items-center gap-4">
                    ...
                    <div>
                        <div class="flex items-center gap-2 mb-1">
                            ...
                            <h1 class="text-xl font-bold text-white tracking-tight">Edit Pengumuman</h1>
                        </div>
                        <p class="text-amber-100/80 text-xs line-clamp-1">{{ announcement.title }}</p>
                    </div>
                </div>
            </div>
        </div>
                ...
                <div class="space-y-5">
                    ...
                    <div class="flex gap-3">
                        <button 
                            ...
                            class="flex-1 py-4 bg-gradient-to-r from-amber-500 to-orange-600 text-white font-bold rounded-xl shadow-lg shadow-amber-500/30 hover:shadow-amber-500/50 hover:-translate-y-0.5 transition-all flex items-center justify-center gap-2 disabled:opacity-50"
                        >
                        ...
                        <Link 
                            ...
                            class="px-6 py-4 bg-gray-100 text-gray-700 font-bold rounded-xl hover:bg-gray-200 transition-all flex items-center"
                        >
                            Batal
                        </Link>
                    </div>
