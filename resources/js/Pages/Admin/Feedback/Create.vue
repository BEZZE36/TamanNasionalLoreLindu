...
const form = useForm({
    display_name: '', destination_id: '', message: '', rating: 5,
    status: 'unread', is_published: false
});

const submit = () => { form.post('/admin/feedback'); };
</script>
    ...
    <div class="max-w-3xl mx-auto space-y-6">
        ...
        <div class="relative overflow-hidden rounded-2xl bg-gradient-to-r from-amber-500 via-orange-500 to-red-500 p-5 shadow-xl">
            ...
            <div class="relative flex items-center gap-3">
                ...
                <div>
                    <h1 class="text-3xl font-extrabold text-white tracking-tight drop-shadow-lg">Tambah Feedback</h1>
                    <p class="mt-1 text-amber-100/90">Buat feedback manual</p>
                </div>
            </div>
        </div>
        ...
        <form @submit.prevent="submit" class="space-y-4">
                ...
                <div class="p-8 space-y-6">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                        <div>
                            ...
                            <input v-model="form.display_name" type="text" required placeholder="Nama pengunjung"
                                class="w-full px-4 py-3 rounded-xl border border-gray-200 focus:border-amber-500 focus:ring-2 focus:ring-amber-500/20">
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-2"><MapPin class="w-4 h-4 inline mr-1" />Destinasi (Opsional)</label>
                    ...
                    <div>
                        ...
                        <textarea v-model="form.message" rows="4" required placeholder="Tulis pesan feedback..."
                ...
                <div class="p-8 space-y-6">
                    ...
                    <div>
                        <label class="flex items-center gap-3 p-4 rounded-xl border-2 border-gray-200 cursor-pointer hover:border-green-300 transition-all">
                            ...
                            <div><span class="font-medium text-gray-700">Tampilkan ke Publik</span><p class="text-xs text-gray-500">Feedback akan ditampilkan di website</p></div>
                        </label>
                    </div>
                </div>
            </div>
            ...
            <div class="flex justify-end">
                <button type="submit" :disabled="form.processing"
                    ...
                    <Save class="w-6 h-6" /><span>{{ form.processing ? 'Menyimpan...' : 'Simpan Feedback' }}</span>
                </button>
            </div>
        </form>
    </div>
