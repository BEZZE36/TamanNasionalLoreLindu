...
import { MessageCircle, Plus, Search, Trash2, Eye, EyeOff, Pin, Reply, ArrowLeft, Filter, Sparkles, AlertTriangle, Loader2, Star } from 'lucide-vue-next';

defineOptions({ layout: AdminLayout });

const props = defineProps({ comments: Object, destinationList: Array, filters: Object });
...
const newComment = ref({ destination_id: '', content: '', rating: 0 });
const hoverRating = ref(0);

const setRating = (r) => { newComment.value.rating = r; };

const stats = computed(() => {
    const data = props.comments?.data || [];
    return { total: data.length, visible: data.filter(c => c.is_visible).length, hidden: data.filter(c => !c.is_visible).length, pinned: data.filter(c => c.is_pinned).length };
});

const search = () => router.get('/admin/destinations/comments', { search: searchQuery.value, destination_id: filterDestination.value }, { preserveState: true });
const openAddModal = () => { replyToId.value = null; newComment.value = { destination_id: '', content: '', rating: 0 }; hoverRating.value = 0; showAddModal.value = true; };
const openReplyModal = (commentId, destId, username) => { replyToId.value = commentId; newComment.value = { destination_id: destId, content: `@${username || 'user'} ` }; showAddModal.value = true; };
...
const submitComment = async () => {
    ...
    try {
        const url = replyToId.value ? `/admin/destinations/comments/${replyToId.value}/reply` : '/admin/destinations/comments';
...
const confirmDelete = async () => {
    try {
        if (deleteType.value === 'single') { await fetch(`/admin/destinations/comments/${deleteId.value}`, { method: 'DELETE', headers: { 'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content } }); }
        else { await fetch('/admin/destinations/comments/batch-delete', { method: 'POST', headers: { 'Content-Type': 'application/json', 'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content }, body: JSON.stringify({ ids: selectedIds.value }) }); }
        showDeleteModal.value = false; router.reload();
    } catch (e) { alert('Error'); }
};

const toggleStatus = async (id) => { await fetch(`/admin/destinations/comments/${id}/toggle-status`, { method: 'POST', headers: { 'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content } }); router.reload(); };
const togglePin = async (id) => { await fetch(`/admin/destinations/comments/${id}/toggle-pin`, { method: 'POST', headers: { 'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content } }); router.reload(); };
const toggleReplyStatus = async (id) => { await fetch(`/admin/destinations/comments/${id}/toggle-status`, { method: 'POST', headers: { 'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content } }); router.reload(); };
const deleteReply = async (id) => { if (!confirm('Hapus balasan?')) return; await fetch(`/admin/destinations/comments/${id}`, { method: 'DELETE', headers: { 'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content } }); router.reload(); };
...
<template>
    <div class="min-h-screen space-y-5">
        <div class="relative overflow-hidden rounded-2xl bg-gradient-to-r from-emerald-500 via-green-500 to-teal-600 p-6 shadow-2xl">
            ...
            <div class="header-content relative flex flex-col lg:flex-row items-start lg:items-center justify-between gap-4">
                <div class="flex items-center gap-4">
                    ...
                    <div><div class="flex items-center gap-2 mb-1"><h1 class="text-xl font-bold text-white tracking-tight drop-shadow-lg">Komentar Destinasi</h1><span class="px-2 py-0.5 bg-white/20 rounded-full text-[10px] font-bold text-white/90">{{ stats.total }} Total</span></div><p class="text-emerald-100/80 text-xs">Kelola komentar destinasi wisata</p></div>
                </div>
                <div class="flex items-center gap-3">
                    <Link href="/admin/destinations" class="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl bg-white/10 text-white text-xs font-semibold border border-white/20 hover:bg-white/20 transition-all"><ArrowLeft class="w-4 h-4" /> Kembali</Link>
        ...
        <div class="grid grid-cols-2 lg:grid-cols-4 gap-3">
            <div class="stat-card rounded-xl p-4 shadow-lg" style="background: linear-gradient(135deg, #10b981 0%, #059669 100%);"><p class="text-2xl font-black text-white">{{ stats.total }}</p><p class="text-[10px] text-white/80">Total</p></div>
        ...
        <div class="rounded-xl bg-white p-4 shadow-lg border border-gray-100">
            <div class="flex flex-col lg:flex-row gap-3">
                ...
                <select v-model="filterDestination" @change="search" class="px-4 py-2.5 text-xs rounded-xl border-2 border-gray-200"><option value="">Semua Destinasi</option><option v-for="d in destinationList" :key="d.id" :value="d.id">{{ d.name }}</option></select>
                    ...
                    <div class="flex items-start gap-3">
                        ...
                        <div class="flex-1 min-w-0">
                            <div class="flex items-center gap-2 flex-wrap">
                                ...
                                <span :class="['px-2 py-0.5 text-[10px] font-bold rounded-full', c.is_visible ? 'bg-emerald-100 text-emerald-600' : 'bg-amber-100 text-amber-600']">{{ c.is_visible ? 'Tampil' : 'Sembunyi' }}</span>
                                <!-- Star Rating -->
                                <div v-if="c.rating && c.rating > 0" class="flex items-center gap-0.5">
                                    <Star v-for="i in 5" :key="i" :class="['w-3 h-3', i <= c.rating ? 'text-amber-400 fill-amber-400' : 'text-gray-200']" />
                                </div>
                            </div>
                            <p class="text-xs text-gray-600 mt-1.5">{{ c.content }}</p>
                            <div class="flex items-center gap-3 mt-2 text-[10px] text-gray-400"><span>{{ formatDate(c.created_at) }}</span><span v-if="c.destination">di <strong class="text-gray-600">{{ c.destination.name }}</strong></span></div>
                            <div v-if="c.replies?.length" class="mt-4 space-y-2 pl-4 border-l-2 border-emerald-200">
                                <div v-for="r in c.replies" :key="r.id" class="flex items-start gap-2 group/r p-2 rounded-lg hover:bg-gray-50">
                                    ...
                                    <div class="flex-1"><div class="flex items-center gap-2"><span class="text-[10px] font-semibold text-gray-800">{{ r.user?.name || r.admin?.name || 'Admin' }}</span><span v-if="!r.is_visible" class="px-1.5 py-0.5 bg-amber-100 text-amber-600 text-[8px] font-bold rounded-full">Hidden</span></div><p class="text-[10px] text-gray-600">{{ r.content }}</p><p class="text-[8px] text-gray-400 mt-0.5">{{ formatDate(r.created_at) }}</p></div>
                                    <div class="flex gap-0.5 opacity-0 group-hover/r:opacity-100"><button @click="toggleReplyStatus(r.id)" class="p-1 rounded text-gray-400 hover:text-emerald-500"><component :is="r.is_visible ? EyeOff : Eye" class="w-3 h-3" /></button><button @click="deleteReply(r.id)" class="p-1 rounded text-gray-400 hover:text-red-500"><Trash2 class="w-3 h-3" /></button></div>
                                </div>
                            </div>
                        </div>
                        <div class="flex items-center gap-1">
                            <button @click="openReplyModal(c.id, c.destination_id, c.user?.name || c.admin?.name || 'user')" class="p-2 rounded-lg text-gray-400 hover:text-blue-600 hover:bg-blue-50"><Reply class="w-4 h-4" /></button>
                            <button @click="toggleStatus(c.id)" :class="c.is_visible ? 'text-emerald-500 hover:bg-amber-50' : 'text-amber-500 hover:bg-emerald-50'" class="p-2 rounded-lg"><component :is="c.is_visible ? EyeOff : Eye" class="w-4 h-4" /></button>
        ...
        <Teleport to="body"><Transition name="modal"><div v-if="showAddModal" class="fixed inset-0 z-50 flex items-center justify-center p-4"><div class="absolute inset-0 bg-black/50 backdrop-blur-sm" @click="showAddModal = false"></div><div class="relative bg-white rounded-2xl shadow-2xl max-w-lg w-full p-6"><h3 class="text-lg font-bold text-gray-900 mb-4 flex items-center gap-2"><MessageCircle class="w-5 h-5 text-emerald-600" />{{ replyToId ? 'Balas Komentar' : 'Tambah Komentar' }}</h3><div v-if="!replyToId" class="mb-4"><label class="block text-xs font-medium text-gray-700 mb-2">Destinasi</label><select v-model="newComment.destination_id" class="w-full px-4 py-2.5 text-xs rounded-xl border-2 border-gray-200"><option value="">Pilih Destinasi...</option><option v-for="d in destinationList" :key="d.id" :value="d.id">{{ d.name }}</option></select></div><div v-if="!replyToId" class="mb-4"><label class="block text-xs font-medium text-gray-700 mb-2">Rating (Opsional)</label><div class="flex items-center gap-1"><button v-for="i in 5" :key="i" @click="setRating(i)" @mouseenter="hoverRating = i" @mouseleave="hoverRating = 0" class="p-0.5 transition-transform hover:scale-110"><Star :class="['w-6 h-6 transition-colors', (hoverRating || newComment.rating) >= i ? 'text-amber-400 fill-amber-400' : 'text-gray-300']" /></button><span v-if="newComment.rating > 0" class="ml-2 text-xs text-amber-600 font-medium">{{ newComment.rating }}/5</span></div></div><div class="mb-6"><label class="block text-xs font-medium text-gray-700 mb-2">Komentar</label><textarea v-model="newComment.content" rows="4" placeholder="Tulis komentar..." class="w-full px-4 py-3 text-xs rounded-xl border-2 border-gray-200 resize-none"></textarea></div><div class="flex gap-3 justify-end"><button @click="showAddModal = false" class="px-4 py-2.5 rounded-xl bg-gray-100 text-gray-700 text-xs font-semibold">Batal</button><button @click="submitComment" :disabled="isSubmitting" class="px-5 py-2.5 rounded-xl bg-emerald-500 text-white text-xs font-semibold flex items-center gap-2 disabled:opacity-50"><Loader2 v-if="isSubmitting" class="w-4 h-4 animate-spin" />{{ isSubmitting ? 'Mengirim...' : 'Kirim' }}</button></div></div></div></Transition></Teleport>
    </div>
</template>
