...
import { MapPin, Save, ArrowLeft, Upload, X, Plus, Trash2, FileText, Image, Pencil, Map, Loader2, Wand2 } from 'lucide-vue-next';
...
const form = useForm({
    name: '', short_description: '', description: '', address: '', city: '', operating_hours: '',
    contact_phone: '', contact_email: '', latitude: '', longitude: '', google_maps_embed: '',
    facilities: [], rules: [], is_active: true, is_featured: false, meta_title: '', meta_description: '', keywords: '',
    cover_image: null, gallery_images: [], prices: [{ ticket_type: 'dewasa', price: '', description: '' }], parking_fees: []
});
...
const handleGalleryUpload = (e) => { 
    ...
    const remaining = 20 - galleryPreviews.value.length;
...
const generateAI = async (type) => {
    ...
    try { if (type === 'seo') { form.meta_title = form.name.substring(0, 60); form.meta_description = `Kunjungi ${form.name} di ${form.city || 'Taman Nasional Lore Lindu'}. ${form.short_description || 'Destinasi wisata menarik dengan keindahan alam.'}`.substring(0, 160); } }
...
const submit = () => { 
    ...
    router.post('/admin/destinations', fd, {
        ...
        <div class="relative overflow-hidden rounded-2xl bg-gradient-to-r from-emerald-500 via-green-500 to-teal-500 p-5 shadow-2xl">
            ...
            <div class="header-content relative flex items-center gap-3">
                ...
                <div class="flex items-center gap-3">
                    ...
                    <div><h1 class="text-lg font-bold text-white flex items-center gap-2"><MapPin class="w-5 h-5" />Tambah Destinasi</h1><p class="text-emerald-100/80 text-xs">Buat destinasi wisata baru</p></div>
                </div>
            </div>
        </div>
        ...
        <form @submit.prevent="submit" class="grid grid-cols-1 lg:grid-cols-3 gap-4">
            <div class="lg:col-span-2 space-y-4">
                ...
                <div class="form-section bg-white rounded-2xl shadow-lg border border-gray-100 p-5">
                    ...
                    <div class="space-y-3">
                        <div>
                            <div class="flex justify-between mb-1"><label class="text-[11px] font-medium text-gray-700">Nama Destinasi *</label><span class="text-[10px] text-gray-400">{{ nameLength }}/100</span></div>
                            <input v-model="form.name" type="text" required maxlength="100" class="w-full px-3 py-2 text-xs rounded-xl border-2 border-gray-200 focus:border-emerald-500">
                        </div>
                ...
                <div class="form-section bg-white rounded-2xl shadow-lg border border-gray-100 p-5">
                    ...
                    <TinyMceEditor v-model="form.description" name="description" id="dest-desc" :height="350" placeholder="Deskripsi destinasi..." />
                </div>
                ...
                <div class="form-section bg-white rounded-2xl shadow-lg border border-gray-100 p-5">
                    ...
                    <div class="space-y-3">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                            <div><label class="block text-[11px] font-medium text-gray-700 mb-1">Latitude</label><input v-model="form.latitude" type="number" step="any" placeholder="-0.8924097" class="w-full px-3 py-2 text-xs rounded-xl border-2 border-gray-200 focus:border-rose-500"></div>
                            <div><label class="block text-[11px] font-medium text-gray-700 mb-1">Longitude</label><input v-model="form.longitude" type="number" step="any" placeholder="119.8295628" class="w-full px-3 py-2 text-xs rounded-xl border-2 border-gray-200 focus:border-rose-500"></div>
                        </div>
                ...
                <div class="form-section bg-white rounded-2xl shadow-lg border border-gray-100 p-5">
                    ...
                    <textarea v-model="form.google_maps_embed" rows="3" class="w-full px-3 py-2 text-[10px] font-mono rounded-xl border-2 border-gray-200 focus:border-teal-500 resize-none" placeholder='<iframe src="https://www.google.com/maps/embed?..."></iframe>'></textarea>
                ...
                <div class="form-section bg-white rounded-2xl shadow-lg border border-gray-100 p-5">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            <label class="block text-[11px] font-medium text-gray-700 mb-2">Fasilitas</label>
                            <div class="flex gap-2 mb-2"><input v-model="newFacility" @keyup.enter.prevent="addFacility" type="text" placeholder="Tambah fasilitas" class="flex-1 px-3 py-2 text-xs rounded-xl border-2 border-gray-200 focus:border-emerald-500"><button type="button" @click="addFacility" class="p-2 bg-emerald-500 text-white rounded-xl hover:bg-emerald-600"><Plus class="w-3 h-3" /></button></div>
                            <div class="flex flex-wrap gap-1"><span v-for="(f, i) in form.facilities" :key="i" class="inline-flex items-center gap-1 px-2 py-0.5 bg-emerald-100 text-emerald-700 rounded-full text-[10px]">{{ f }}<button type="button" @click="removeFacility(i)" class="hover:text-emerald-900"><X class="w-2.5 h-2.5" /></button></span></div>
                        </div>
                        <div>
                            <label class="block text-[11px] font-medium text-gray-700 mb-2">Aturan</label>
                            <div class="flex gap-2 mb-2"><input v-model="newRule" @keyup.enter.prevent="addRule" type="text" placeholder="Tambah aturan" class="flex-1 px-3 py-2 text-xs rounded-xl border-2 border-gray-200 focus:border-emerald-500"><button type="button" @click="addRule" class="p-2 bg-amber-500 text-white rounded-xl hover:bg-amber-600"><Plus class="w-3 h-3" /></button></div>
                            <div class="flex flex-wrap gap-1"><span v-for="(r, i) in form.rules" :key="i" class="inline-flex items-center gap-1 px-2 py-0.5 bg-amber-100 text-amber-700 rounded-full text-[10px]">{{ r }}<button type="button" @click="removeRule(i)" class="hover:text-amber-900"><X class="w-2.5 h-2.5" /></button></span></div>
                        </div>
                    </div>
                </div>
                ...
                <div class="form-section bg-white rounded-2xl shadow-lg border border-gray-100 p-5">
                    ...
                    <div class="space-y-2">
                        <div v-for="(price, i) in form.prices" :key="i" class="grid grid-cols-1 md:grid-cols-4 gap-2 p-3 bg-gray-50 rounded-xl">
                            <input v-model="price.ticket_type" type="text" placeholder="Jenis tiket" class="px-3 py-2 text-xs rounded-xl border-2 border-gray-200 focus:border-emerald-500">
                            <input v-model="price.price" type="number" placeholder="Harga" class="px-3 py-2 text-xs rounded-xl border-2 border-gray-200 focus:border-emerald-500">
                            <input v-model="price.description" type="text" placeholder="Keterangan" class="px-3 py-2 text-xs rounded-xl border-2 border-gray-200 focus:border-emerald-500">
                            <button v-if="form.prices.length > 1" type="button" @click="removePrice(i)" class="p-2 text-red-500 hover:bg-red-50 rounded-xl self-center"><Trash2 class="w-3 h-3" /></button>
                        </div>
                    </div>
                </div>
                ...
                <div class="form-section bg-white rounded-2xl shadow-lg border border-gray-100 p-5">
                    ...
                    <div class="space-y-2">
                        <div v-for="(fee, i) in form.parking_fees" :key="i" class="grid grid-cols-1 md:grid-cols-4 gap-2 p-3 bg-blue-50 rounded-xl">
                            <input v-model="fee.vehicle_type" type="text" placeholder="Jenis kendaraan" class="px-3 py-2 text-xs rounded-xl border-2 border-gray-200 focus:border-blue-500">
                            <input v-model="fee.price" type="number" placeholder="Biaya parkir" class="px-3 py-2 text-xs rounded-xl border-2 border-gray-200 focus:border-blue-500">
                            <input v-model="fee.description" type="text" placeholder="Keterangan" class="px-3 py-2 text-xs rounded-xl border-2 border-gray-200 focus:border-blue-500">
                            <button type="button" @click="form.parking_fees.splice(i, 1)" class="p-2 text-red-500 hover:bg-red-50 rounded-xl self-center"><Trash2 class="w-3 h-3" /></button>
                        </div>
            ...
            <div class="space-y-4">
                ...
                <div class="form-section bg-white rounded-2xl shadow-lg border border-gray-100 p-5">
                    <h2 class="text-sm font-bold text-gray-900 mb-3 flex items-center gap-2"><Image class="w-4 h-4 text-teal-600" />Gambar Utama</h2>
                    <label class="block cursor-pointer" @dragover.prevent="isDraggingCover = true" @dragleave.prevent="isDraggingCover = false" @drop.prevent="handleCoverDrop">
                        <div v-if="!coverPreview" :class="[isDraggingCover ? 'border-emerald-500 bg-emerald-50 scale-[1.02]' : 'border-gray-300 hover:border-emerald-400']" class="border-2 border-dashed rounded-xl p-6 text-center transition-all"><Upload class="w-8 h-8 mx-auto text-gray-400 mb-2" /><p class="text-xs text-gray-600 font-medium">Drag & drop</p></div>
                        <input type="file" accept=".jpg,.jpeg,.png,.webp" @change="handleCoverUpload" class="hidden">
                    </label>
                    <div v-if="coverPreview" class="mt-3 relative rounded-xl overflow-hidden shadow-lg group"><img :src="coverPreview" class="w-full h-36 object-cover"><div class="absolute inset-0 bg-black/40 flex items-center justify-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity"><button type="button" @click.prevent="openCoverEditor" class="p-2 bg-blue-500 text-white rounded-xl hover:bg-blue-600"><Pencil class="w-4 h-4" /></button><button type="button" @click.prevent="removeCover" class="p-2 bg-red-500 text-white rounded-xl hover:bg-red-600"><X class="w-4 h-4" /></button></div></div>
                </div>
                ...
                <div class="form-section bg-white rounded-2xl shadow-lg border border-gray-100 p-5">
                    ...
                    <div v-if="galleryPreviews.length" class="grid grid-cols-3 gap-2 mb-3"><div v-for="(img, i) in galleryPreviews" :key="i" class="relative aspect-square group rounded-lg overflow-hidden"><img :src="img" class="w-full h-full object-cover"><div class="absolute inset-0 bg-black/40 flex items-center justify-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity"><button type="button" @click="openGalleryEditor(i)" class="p-1 bg-blue-500 text-white rounded-lg"><Pencil class="w-3 h-3" /></button><button type="button" @click="removeGalleryImage(i)" class="p-1 bg-red-500 text-white rounded-lg"><X class="w-3 h-3" /></button></div></div></div>
                    <label v-if="galleryPreviews.length < 20" class="block cursor-pointer" @dragover.prevent="isDraggingGallery = true" @dragleave.prevent="isDraggingGallery = false" @drop.prevent="handleGalleryDrop"><div :class="[isDraggingGallery ? 'border-purple-500 bg-purple-50' : 'border-gray-200 hover:border-purple-400']" class="border-2 border-dashed rounded-xl p-4 text-center transition-all"><Plus class="w-5 h-5 mx-auto text-gray-400" /><p class="text-[10px] text-gray-500 mt-1">Tambah Foto (max 20)</p></div><input type="file" accept=".jpg,.jpeg,.png,.webp" multiple @change="handleGalleryUpload" class="hidden"></label>
