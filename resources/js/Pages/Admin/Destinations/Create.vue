...
import { MapPin, Save, ArrowLeft, Upload, X, Plus, Trash2, Info, FileText, Sparkles, Image, Pencil, Map, Loader2 } from 'lucide-vue-next';
...
const form = useForm({
    ...
    cover_image: null,
    gallery_images: [],
    prices: [{ ticket_type: 'dewasa', price: '', description: '' }],
    parking_fees: []
});
...
const removeFacility = (index) => form.facilities.splice(index, 1);
...
const removePrice = (index) => form.prices.splice(index, 1);
...
const handleEditorSave = async (dataUrl) => {
    ...
    if (editingImageType.value === 'cover') { form.cover_image = file; coverPreview.value = dataUrl; }
    else { form.gallery_images[editingGalleryIndex.value] = file; galleryPreviews.value[editingGalleryIndex.value] = dataUrl; }
...
const submit = () => {
    ...
    if (window.tinymce) {
        ...
        const editor = window.tinymce.get('dest-desc');
        if (editor) {
    ...
    <div class="space-y-4">
        <div class="flex items-center justify-between">
            <div class="flex items-center gap-3">
                <Link href="/admin/destinations" class="p-2 rounded-xl bg-gray-100 hover:bg-gray-200 transition-colors"><ArrowLeft class="w-5 h-5 text-gray-600" /></Link>
                <div>
                    <h1 class="text-lg font-bold text-gray-900">Tambah Destinasi</h1>
                    <p class="text-gray-500 text-sm">Buat destinasi wisata baru</p>
                </div>
            </div>
        </div>
        ...
        <form @submit.prevent="submit" class="grid grid-cols-1 lg:grid-cols-3 gap-3">
            <div class="lg:col-span-2 space-y-6">
                ...
                <div class="bg-white rounded-2xl shadow-lg border border-gray-100 p-6">
                    <h2 class="text-lg font-bold text-gray-900 mb-6 flex items-center gap-2"><MapPin class="w-5 h-5 text-emerald-600" />Informasi Dasar</h2>
                    <div class="space-y-4">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-1">Nama Destinasi *</label>
                                <input v-model="form.name" type="text" required class="w-full px-4 py-2.5 rounded-xl border border-gray-200 focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500/20">
                                <p v-if="form.errors.name" class="text-red-500 text-xs mt-1">{{ form.errors.name }}</p>
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-1">Kategori</label>
                                <select v-model="form.category_id" class="w-full px-4 py-2.5 rounded-xl border border-gray-200 focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500/20">
                        ...
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">Deskripsi Singkat <span class="text-gray-400 font-normal">(maks 255 karakter)</span></label>
                            <input v-model="form.short_description" type="text" maxlength="255" class="w-full px-4 py-2.5 rounded-xl border border-gray-200 focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500/20">
                            <p class="text-xs text-gray-400 mt-1">{{ form.short_description?.length || 0 }}/255 karakter</p>
                        </div>
                    </div>
                </div>
                ...
                <div class="bg-white/80 backdrop-blur-xl rounded-2xl shadow-lg shadow-gray-200/50 border border-gray-100/50 p-6">
                    ...
                    <TinyMceEditor v-model="form.description" name="description" id="dest-desc" :height="400" placeholder="Deskripsi destinasi..." />
                </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                        <div>
                            ...
                            <div class="flex gap-2 mb-3">
                                <input v-model="newFacility" @keyup.enter.prevent="addFacility" type="text" placeholder="Tambah fasilitas" class="flex-1 px-4 py-2 rounded-xl border border-gray-200 focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500/20">
                                <button type="button" @click="addFacility" class="p-2 bg-emerald-500 text-white rounded-xl hover:bg-emerald-600"><Plus class="w-4 h-4" /></button>
                            </div>
                            <div class="flex flex-wrap gap-2">
                                <span v-for="(f, i) in form.facilities" :key="i" class="inline-flex items-center gap-1 px-3 py-1 bg-emerald-100 text-emerald-700 rounded-full text-sm">
                                    {{ f }}<button type="button" @click="removeFacility(i)" class="hover:text-emerald-900"><X class="w-3 h-3" /></button>
                                </span>
                            </div>
                        </div>
                        <div>
                            ...
                            <div class="flex gap-2 mb-3">
                                <input v-model="newRule" @keyup.enter.prevent="addRule" type="text" placeholder="Tambah aturan" class="flex-1 px-4 py-2 rounded-xl border border-gray-200 focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500/20">
                                <button type="button" @click="addRule" class="p-2 bg-emerald-500 text-white rounded-xl hover:bg-emerald-600"><Plus class="w-4 h-4" /></button>
                            </div>
                            <div class="flex flex-wrap gap-2">
                                <span v-for="(r, i) in form.rules" :key="i" class="inline-flex items-center gap-1 px-3 py-1 bg-amber-100 text-amber-700 rounded-full text-sm">
                                    {{ r }}<button type="button" @click="removeRule(i)" class="hover:text-amber-900"><X class="w-3 h-3" /></button>
                                </span>
                            </div>
                        </div>
                    </div>
                ...
                <div class="bg-white rounded-2xl shadow-lg border border-gray-100 p-6">
                    <div class="flex items-center justify-between mb-6">
                        <h2 class="text-lg font-bold text-gray-900">Harga Tiket</h2>
                        <button type="button" @click="addPrice" class="px-3 py-1.5 bg-emerald-500 text-white rounded-xl text-sm font-medium hover:bg-emerald-600 flex items-center gap-1"><Plus class="w-4 h-4" />Tambah</button>
                    </div>
                    <div class="space-y-4">
                        <div v-for="(price, i) in form.prices" :key="i" class="grid grid-cols-1 md:grid-cols-4 gap-3 p-4 bg-gray-50 rounded-xl">
                            <input v-model="price.ticket_type" type="text" placeholder="Jenis tiket" class="px-4 py-2 rounded-xl border border-gray-200 focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500/20">
                            <input v-model="price.price" type="number" placeholder="Harga" class="px-4 py-2 rounded-xl border border-gray-200 focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500/20">
                            <input v-model="price.description" type="text" placeholder="Keterangan" class="px-4 py-2 rounded-xl border border-gray-200 focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500/20">
                            <button v-if="form.prices.length > 1" type="button" @click="removePrice(i)" class="p-2 text-red-500 hover:bg-red-50 rounded-xl self-center"><Trash2 class="w-4 h-4" /></button>
                        </div>
                    </div>
                </div>
                ...
                <div class="bg-white rounded-2xl shadow-lg border border-gray-100 p-6">
                    <div class="flex items-center justify-between mb-6">
                        <h2 class="text-lg font-bold text-gray-900">Biaya Parkir</h2>
                        <button type="button" @click="form.parking_fees.push({ vehicle_type: '', price: '', description: '' })" class="px-3 py-1.5 bg-blue-500 text-white rounded-xl text-sm font-medium hover:bg-blue-600 flex items-center gap-1"><Plus class="w-4 h-4" />Tambah</button>
                    </div>
                    <div class="space-y-4">
                        <div v-for="(fee, i) in form.parking_fees" :key="i" class="grid grid-cols-1 md:grid-cols-4 gap-3 p-4 bg-blue-50 rounded-xl">
                            <input v-model="fee.vehicle_type" type="text" placeholder="Jenis kendaraan (Motor/Mobil)" class="px-4 py-2 rounded-xl border border-gray-200 focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20">
                            <input v-model="fee.price" type="number" placeholder="Biaya parkir" class="px-4 py-2 rounded-xl border border-gray-200 focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20">
                            <input v-model="fee.description" type="text" placeholder="Keterangan (opsional)" class="px-4 py-2 rounded-xl border border-gray-200 focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20">
                            <button type="button" @click="form.parking_fees.splice(i, 1)" class="p-2 text-red-500 hover:bg-red-50 rounded-xl self-center"><Trash2 class="w-4 h-4" /></button>
                        </div>
                        <p v-if="!form.parking_fees.length" class="text-center py-4 text-gray-400 text-sm">Belum ada biaya parkir. Klik "Tambah" untuk menambahkan.</p>
                    </div>
                </div>
            </div>
            ...
            <div class="space-y-4">
                ...
                <!-- Cover Image -->
                <div class="bg-white/80 backdrop-blur-xl rounded-2xl shadow-lg shadow-gray-200/50 border border-gray-100/50 p-6">
                    <div class="flex items-center gap-3 mb-6"><div class="w-10 h-10 bg-gradient-to-br from-teal-400/20 to-green-500/20 rounded-xl flex items-center justify-center"><Image class="w-5 h-5 text-teal-600" /></div><h2 class="text-lg font-bold text-gray-900">Gambar Utama</h2></div>
                    <label class="block cursor-pointer" @dragover.prevent="isDraggingCover = true" @dragleave.prevent="isDraggingCover = false" @drop.prevent="handleCoverDrop">
                        <div v-if="!coverPreview" :class="[isDraggingCover ? 'border-emerald-500 bg-emerald-50 scale-[1.02]' : 'border-gray-300 hover:border-emerald-400']" class="border-2 border-dashed rounded-xl p-8 text-center transition-all"><Upload class="w-12 h-12 mx-auto text-gray-400 mb-3" /><p class="text-sm text-gray-600">Drag & drop gambar</p><p class="text-xs text-gray-400 mt-1">Max 2MB • JPG, PNG</p></div>
                        <input type="file" accept="image/*" @change="handleCoverUpload" class="hidden">
                    </label>
                    <div v-if="coverPreview" class="mt-4"><div class="relative rounded-xl overflow-hidden shadow-lg border border-gray-100 group"><img :src="coverPreview" class="w-full h-48 object-cover"><div class="absolute inset-0 bg-black/40 flex items-center justify-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity"><button type="button" @click.prevent="openCoverEditor" class="bg-blue-500 text-white p-2.5 rounded-xl hover:bg-blue-600"><Pencil class="w-5 h-5" /></button><button type="button" @click.prevent="removeCover" class="bg-red-500 text-white p-2.5 rounded-xl hover:bg-red-600"><X class="w-5 h-5" /></button></div></div></div>
                </div>

                <!-- Gallery -->
                <div class="bg-white/80 backdrop-blur-xl rounded-2xl shadow-lg shadow-gray-200/50 border border-gray-100/50 p-6">
                    <div class="flex items-center gap-3 mb-6"><div class="w-10 h-10 bg-gradient-to-br from-pink-400/20 to-rose-500/20 rounded-xl flex items-center justify-center"><svg class="w-5 h-5 text-pink-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z"/></svg></div><h2 class="text-lg font-bold text-gray-900">Galeri Foto</h2></div>
                ...
                <div class="bg-white rounded-2xl shadow-lg border border-gray-100 p-6">
                    ...
                    <div class="space-y-4">
                        <div>
                            ...
                            <input v-model="form.operating_hours" type="text" placeholder="08:00 - 17:00" class="w-full px-4 py-2.5 rounded-xl border border-gray-200 focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500/20">
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">Telepon</label>
                            <input v-model="form.contact_phone" type="tel" class="w-full px-4 py-2.5 rounded-xl border border-gray-200 focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500/20">
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">Email</label>
                            <input v-model="form.contact_email" type="email" class="w-full px-4 py-2.5 rounded-xl border border-gray-200 focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500/20">
                        </div>
                    </div>
                </div>

                <!-- Submit -->
                <button type="submit" :disabled="form.processing" class="w-full py-3.5 bg-gradient-to-r from-emerald-500 to-green-600 text-white font-bold rounded-xl shadow-lg shadow-emerald-500/30 hover:shadow-emerald-500/50 hover:-translate-y-0.5 transition-all flex items-center justify-center gap-2 disabled:opacity-50">
                    <Save class="w-5 h-5" />{{ form.processing ? 'Menyimpan...' : 'Simpan Destinasi' }}
                </button>
            </div>
        </form>
