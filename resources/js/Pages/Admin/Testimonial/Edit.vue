<script setup>
/**
 * Admin Testimonial Edit - Premium Design
...
import { 
    ...
    User, MessageSquare, Sparkles, Loader2, Edit
...
const submit = () => { form.put(`/admin/testimonial/${props.feedback.id}`); };
...
<template>
    <div class="max-w-2xl mx-auto space-y-4">
        <!-- Premium Header -->
        <div class="form-card relative overflow-hidden rounded-xl bg-gradient-to-r from-amber-500 via-orange-500 to-rose-500 p-4 shadow-xl">
            <!-- Animated Background -->
            <div class="absolute inset-0 overflow-hidden">
                <div class="absolute -top-10 -right-10 w-32 h-32 bg-white/10 rounded-full blur-2xl animate-pulse"></div>
                <div class="absolute bottom-0 left-1/4 w-24 h-24 bg-orange-400/20 rounded-full blur-xl animate-pulse" style="animation-delay: 0.5s"></div>
            </div>
            
            <!-- Floating Particles -->
            <div class="absolute top-3 right-16 w-1.5 h-1.5 bg-white/40 rounded-full animate-bounce" style="animation-delay: 0.2s"></div>
            <div class="absolute bottom-4 right-1/3 w-1 h-1 bg-amber-300/50 rounded-full animate-bounce" style="animation-delay: 0.6s"></div>
            ...
            <div class="header-content relative flex items-center gap-3">
                ...
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        <h1 class="text-lg font-bold text-white tracking-tight">Edit Testimonial</h1>
                        <p class="text-[10px] text-amber-100/80">{{ feedback.display_name }}</p>
                    </div>
                </div>
            </div>
        </div>
        ...
        <form @submit.prevent="submit" class="space-y-3">
            ...
            <div class="form-card bg-white rounded-xl shadow-lg border border-gray-100 overflow-hidden">
                <div class="px-4 py-3 bg-gradient-to-r from-amber-50 to-orange-50 border-b border-gray-100">
                ...
                <div class="p-4 space-y-4">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                        <div>
                            ...
                            <input 
                                ...
                                class="w-full px-3 py-2 text-xs rounded-lg border-2 border-gray-200 bg-gray-50/50 focus:border-amber-500 focus:ring-2 focus:ring-amber-500/10 focus:bg-white transition-all"
                            >
                        ...
                        <div>
                            ...
                            <select 
                                ...
                                class="w-full px-3 py-2 text-xs rounded-lg border-2 border-gray-200 bg-gray-50/50 focus:border-amber-500 focus:ring-2 focus:ring-amber-500/10 focus:bg-white transition-all cursor-pointer"
                            >
                    ...
                    <div>
                        ...
                        <textarea 
                            ...
                            class="w-full px-3 py-2 text-xs rounded-lg border-2 border-gray-200 bg-gray-50/50 focus:border-amber-500 focus:ring-2 focus:ring-amber-500/10 focus:bg-white transition-all resize-none"
                        ></textarea>
            ...
            <div class="form-card flex items-center justify-between pt-2">
                ...
                <button 
                    ...
                    class="group inline-flex items-center gap-2 px-5 py-2.5 rounded-xl bg-gradient-to-r from-amber-500 to-orange-500 text-white text-xs font-bold shadow-lg shadow-amber-500/30 hover:shadow-xl hover:shadow-amber-500/40 hover:-translate-y-0.5 transition-all duration-300 disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:translate-y-0"
                >
                    <Loader2 v-if="form.processing" class="w-4 h-4 animate-spin" />
                    <Save v-else class="w-4 h-4 group-hover:scale-110 transition-transform" />
                    <span>{{ form.processing ? 'Menyimpan...' : 'Perbarui Testimonial' }}</span>
