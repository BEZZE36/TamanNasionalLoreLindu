...
import { Bird, Eye, Heart, MessageCircle, Star, TrendingUp, BarChart3, Sparkles, Shield, Leaf } from 'lucide-vue-next';
...
const props = defineProps({
    ...
    topFauna: { type: Array, default: () => [] },
    recentFauna: { type: Array, default: () => [] },
...
const changePeriod = () => router.get('/admin/fauna/dashboard', { period: period.value }, { preserveState: true });
    ...
    <div class="min-h-screen space-y-5">
        ...
        <div class="relative overflow-hidden rounded-2xl bg-gradient-to-r from-amber-500 via-orange-500 to-rose-500 p-6 shadow-2xl">
            ...
            <div class="header-content relative flex flex-col lg:flex-row items-start lg:items-center justify-between gap-4">
                <div class="flex items-center gap-4">
                    ...
                    <div>
                        <div class="flex items-center gap-2 mb-1">
                            <h1 class="text-xl font-bold text-white tracking-tight drop-shadow-lg">Dashboard Fauna</h1>
                            <span class="px-2 py-0.5 bg-white/20 rounded-full text-[10px] font-bold text-white/90 backdrop-blur-sm">
                                {{ formatNumber(stats?.total || 0) }} Spesies
                            </span>
                        </div>
                        <p class="text-amber-100/80 text-xs">Statistik dan analitik data fauna</p>
                    </div>
                </div>
                ...
                <div class="flex items-center gap-3">
                    ...
                    <Link href="/admin/fauna" class="group inline-flex items-center gap-2 px-5 py-2.5 rounded-xl bg-white text-amber-600 text-xs font-bold hover:bg-amber-50 hover:shadow-xl hover:-translate-y-0.5 transition-all duration-300 shadow-lg">
                        <Bird class="w-4 h-4 group-hover:scale-110 transition-transform" />
        ...
        <div class="grid grid-cols-2 lg:grid-cols-5 gap-3">
            <div class="stat-card group relative overflow-hidden rounded-xl bg-white p-4 shadow-lg border border-gray-100 hover:shadow-xl hover:-translate-y-1 transition-all duration-300">
                ...
                <div class="relative flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p class="text-[10px] text-gray-500 font-medium">Total Fauna</p>
                    </div>
                </div>
            </div>
            ...
            <div class="stat-card group relative overflow-hidden rounded-xl bg-white p-4 shadow-lg border border-gray-100 hover:shadow-xl hover:-translate-y-1 transition-all duration-300">
                ...
                <div class="relative flex items-center gap-3">
                    ...
                    <div>
                        <p class="text-xl font-black text-gray-900">{{ stats?.endangered || 0 }}</p>
                        <p class="text-[10px] text-gray-500 font-medium">Langka</p>
                    </div>
                </div>
            </div>
            
            <div class="stat-card group relative overflow-hidden rounded-xl bg-white p-4 shadow-lg border border-gray-100 hover:shadow-xl hover:-translate-y-1 transition-all duration-300">
                <div class="absolute top-0 right-0 w-20 h-20 bg-gradient-to-br from-purple-100 to-violet-100 rounded-full -translate-y-1/2 translate-x-1/2 opacity-50 group-hover:opacity-100 transition-opacity"></div>
                <div class="relative flex items-center gap-3">
                    <div class="flex h-10 w-10 items-center justify-center rounded-xl bg-gradient-to-br from-purple-500 to-violet-600 shadow-lg shadow-purple-500/30 group-hover:scale-110 transition-transform">
                        <Leaf class="w-5 h-5 text-white" />
                    </div>
                    <div>
                        <p class="text-xl font-black text-gray-900">{{ stats?.endemic || 0 }}</p>
                        <p class="text-[10px] text-gray-500 font-medium">Endemik</p>
                    </div>
                </div>
            </div>
        ...
        <div class="grid grid-cols-3 gap-3">
            ...
            <div class="status-card group relative overflow-hidden bg-gradient-to-br from-blue-500 to-cyan-500 rounded-xl p-4 text-white shadow-lg hover:shadow-xl hover:-translate-y-1 transition-all duration-300">
                <div class="absolute -top-10 -right-10 w-24 h-24 bg-white/10 rounded-full blur-2xl"></div>
                <p class="text-2xl font-black">{{ formatNumber(stats?.total_comments || 0) }}</p>
                <p class="text-xs font-medium opacity-80">Komentar</p>
            </div>
        </div>
        ...
        <div class="grid lg:grid-cols-2 gap-5">
            ...
            <div class="chart-section rounded-xl bg-white p-5 shadow-lg border border-gray-100">
                ...
                <div v-if="viewsData.length > 0" class="h-48 flex items-end gap-1 px-2">
                    <div v-for="(val, i) in viewsData" :key="i" 
                        class="flex-1 bg-gradient-to-t from-amber-500 to-orange-400 rounded-t-md hover:from-amber-600 hover:to-orange-500 transition-all cursor-pointer relative group min-w-[8px]"
            ...
            <div class="chart-section rounded-xl bg-white p-5 shadow-lg border border-gray-100">
                <h3 class="text-sm font-bold text-gray-900 mb-4 flex items-center gap-2">
                    ...
                    Top 10 Fauna Populer
                </h3>
                <div class="space-y-2.5 max-h-48 overflow-y-auto">
                    <div v-for="(fauna, i) in topFauna" :key="fauna.id" class="flex items-center gap-3 group p-2 rounded-lg hover:bg-gray-50 transition-colors">
                        <span class="w-6 h-6 bg-gradient-to-br from-amber-500 to-orange-600 text-white rounded-full flex items-center justify-center text-[10px] font-bold shadow-sm">{{ i + 1 }}</span>
                        <div class="flex-1 min-w-0">
                            <Link :href="`/admin/fauna/${fauna.id}/edit`" class="text-xs font-medium text-gray-900 group-hover:text-amber-600 truncate block transition-colors">
                                {{ fauna.name }}
                            </Link>
                            <p class="text-[10px] text-gray-400">{{ formatNumber(fauna.views_count || 0) }} views • {{ fauna.category }}</p>
                        </div>
                    </div>
                    <p v-if="!topFauna?.length" class="text-gray-400 text-xs text-center py-4">Belum ada data</p>
                </div>
            </div>
        </div>
        ...
        <div class="bottom-section rounded-xl bg-white p-5 shadow-lg border border-gray-100">
            <h3 class="text-sm font-bold text-gray-900 mb-4 flex items-center gap-2">
                <div class="flex h-8 w-8 items-center justify-center rounded-lg bg-gradient-to-br from-blue-500 to-cyan-600 shadow-md">
                    <Bird class="w-4 h-4 text-white" />
                </div>
                Fauna Terbaru
            </h3>
            <div class="grid grid-cols-2 lg:grid-cols-4 gap-3">
                <div v-for="fauna in recentFauna" :key="fauna.id" class="group rounded-xl overflow-hidden border border-gray-100 hover:shadow-lg transition-all">
                    <div class="aspect-video bg-gray-100 relative">
                        <img v-if="fauna.image_url" :src="fauna.image_url" class="w-full h-full object-cover" />
                        <div class="absolute top-2 right-2">
                            <span :class="['px-1.5 py-0.5 rounded text-[8px] font-bold', fauna.category === 'endemik' ? 'bg-purple-500 text-white' : fauna.category === 'langka' ? 'bg-red-500 text-white' : 'bg-gray-500 text-white']">
                                {{ fauna.category }}
                            </span>
                        </div>
                    </div>
                    <div class="p-3">
                        <Link :href="`/admin/fauna/${fauna.id}/edit`" class="text-xs font-bold text-gray-900 group-hover:text-amber-600 truncate block">{{ fauna.name }}</Link>
                        <p class="text-[10px] text-gray-400 mt-0.5 italic">{{ fauna.scientific_name }}</p>
                    </div>
                </div>
                <p v-if="!recentFauna?.length" class="col-span-4 text-gray-400 text-xs text-center py-8">Belum ada fauna</p>
            </div>
        </div>
    </div>
</template>
