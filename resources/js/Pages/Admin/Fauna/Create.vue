...
const handleEditorSave = async (dataUrl) => {
    ...
    if (editingImageType.value === 'cover') { form.image = file; imagePreview.value = dataUrl; }
    else { form.gallery[editingGalleryIndex.value] = file; galleryPreviews.value[editingGalleryIndex.value] = dataUrl; }
    showImageEditor.value = false;
};

const submit = () => form.post('/admin/fauna', { forceFormData: true, onSuccess: () => router.visit('/admin/fauna') });
</script>
...
<template>
    <div class="space-y-4">
        <div class="flex items-center gap-3">
            ...
            <div class="flex items-center gap-3">
                <div class="w-14 h-14 bg-gradient-to-br from-amber-400 to-orange-600 rounded-2xl flex items-center justify-center shadow-lg shadow-amber-500/30"><Plus class="w-7 h-7 text-white" /></div>
                <div><h1 class="text-2xl font-black text-gray-900">Tambah Fauna</h1><p class="text-gray-500 text-sm">Tambahkan data fauna atau satwa baru</p></div>
            </div>
        </div>
        ...
        <form @submit.prevent="submit" class="grid grid-cols-1 xl:grid-cols-3 gap-3">
            <div class="xl:col-span-2 space-y-6">
                ...
                <div class="bg-white/80 backdrop-blur-xl rounded-2xl shadow-lg shadow-gray-200/50 border border-gray-100/50 p-6">
                    ...
                    <div class="space-y-5">
                        <div><label class="block text-sm font-semibold text-gray-700 mb-2">Nama Fauna <span class="text-red-500">*</span></label><input v-model="form.name" type="text" required placeholder="Contoh: Anoa" class="w-full px-4 py-3 rounded-xl bg-gray-50 border border-gray-200 focus:bg-white focus:border-amber-500 focus:ring-2 focus:ring-amber-500/20 transition-all"></div>
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                            <div><label class="block text-sm font-semibold text-gray-700 mb-2">Nama Ilmiah</label><input v-model="form.scientific_name" type="text" placeholder="Contoh: Bubalus depressicornis" class="w-full px-4 py-3 rounded-xl bg-gray-50 border border-gray-200 focus:bg-white focus:border-amber-500 focus:ring-2 focus:ring-amber-500/20 transition-all italic"></div>
                            <div><label class="block text-sm font-semibold text-gray-700 mb-2">Nama Lokal</label><input v-model="form.local_name" type="text" placeholder="Contoh: Sapi Hutan" class="w-full px-4 py-3 rounded-xl bg-gray-50 border border-gray-200 focus:bg-white focus:border-amber-500 focus:ring-2 focus:ring-amber-500/20 transition-all"></div>
                        </div>
                        <div><label class="block text-sm font-semibold text-gray-700 mb-2">Habitat</label><input v-model="form.habitat" type="text" placeholder="Contoh: Hutan hujan tropis" class="w-full px-4 py-3 rounded-xl bg-gray-50 border border-gray-200 focus:bg-white focus:border-amber-500 focus:ring-2 focus:ring-amber-500/20 transition-all"><Home class="w-4 h-4 text-gray-400 absolute right-4 top-1/2 -translate-y-1/2" /></div>
                    </div>
                </div>
                ...
                <div class="bg-white/80 backdrop-blur-xl rounded-2xl shadow-lg shadow-gray-200/50 border border-gray-100/50 p-6">
                    ...
                    <TinyMceEditor v-model="form.description" name="description" id="fauna-desc" :height="400" placeholder="Deskripsi fauna..." />
                </div>
                ...
                <div class="bg-white/80 backdrop-blur-xl rounded-2xl shadow-lg shadow-gray-200/50 border border-gray-100/50 p-6">
                    ...
                    <div class="space-y-4">
                        <div><label class="block text-sm font-semibold text-gray-700 mb-2">Meta Title</label><input v-model="form.meta_title" type="text" maxlength="60" placeholder="Judul untuk mesin pencari" class="w-full px-4 py-3 rounded-xl bg-gray-50 border border-gray-200 focus:bg-white focus:border-amber-500 focus:ring-2 focus:ring-amber-500/20"><p class="text-xs text-gray-400 mt-1">{{ form.meta_title?.length || 0 }}/60</p></div>
                        <div><label class="block text-sm font-semibold text-gray-700 mb-2">Meta Description</label><textarea v-model="form.meta_description" rows="3" maxlength="160" placeholder="Deskripsi singkat" class="w-full px-4 py-3 rounded-xl bg-gray-50 border border-gray-200 focus:bg-white focus:border-amber-500 focus:ring-2 focus:ring-amber-500/20 resize-none"></textarea><p class="text-xs text-gray-400 mt-1">{{ form.meta_description?.length || 0 }}/160</p></div>
                    </div>
                </div>
            </div>
            ...
            <div class="space-y-4">
                ...
                <div class="bg-white/80 backdrop-blur-xl rounded-2xl shadow-lg shadow-gray-200/50 border border-gray-100/50 p-6">
                    ...
                    <button type="submit" :disabled="form.processing" class="w-full mt-6 py-3.5 bg-gradient-to-r from-amber-500 to-orange-600 text-white font-bold rounded-xl shadow-lg shadow-amber-500/30 hover:shadow-amber-500/50 hover:-translate-y-0.5 transition-all flex items-center justify-center gap-2 disabled:opacity-50"><Save class="w-5 h-5" />{{ form.processing ? 'Menyimpan...' : 'Simpan Fauna' }}</button>
                </div>

                <!-- Cover Image -->
                <div class="bg-white/80 backdrop-blur-xl rounded-2xl shadow-lg shadow-gray-200/50 border border-gray-100/50 p-6">
                    <div class="flex items-center gap-3 mb-6"><div class="w-10 h-10 bg-gradient-to-br from-teal-400/20 to-green-500/20 rounded-xl flex items-center justify-center"><Image class="w-5 h-5 text-teal-600" /></div><div><h2 class="text-lg font-bold text-gray-900">Gambar Utama</h2><p class="text-xs text-gray-500">Thumbnail fauna</p></div></div>
                    <label class="block cursor-pointer" @dragover.prevent="isDraggingCover = true" @dragleave.prevent="isDraggingCover = false" @drop.prevent="handleCoverDrop">
                        <div v-if="!imagePreview" :class="[isDraggingCover ? 'border-amber-500 bg-amber-50 scale-[1.02]' : 'border-gray-300 hover:border-amber-400']" class="border-2 border-dashed rounded-xl p-8 text-center transition-all"><Upload class="w-12 h-12 mx-auto text-gray-400 mb-3" /><p class="text-sm text-gray-600 font-medium">Drag & drop gambar</p><p class="text-xs text-gray-400 mt-1">Max 2MB • JPG, PNG</p></div>
                        <input type="file" accept="image/*" class="hidden" @change="handleCoverUpload">
                    </label>
                    <div v-if="imagePreview" class="mt-4"><div class="relative rounded-xl overflow-hidden shadow-lg border border-gray-100 group"><img :src="imagePreview" class="w-full h-48 object-cover"><div class="absolute inset-0 bg-black/40 flex items-center justify-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity"><button type="button" @click.prevent="openCoverEditor" class="bg-blue-500 text-white p-2.5 rounded-xl hover:bg-blue-600" title="Edit"><Pencil class="w-5 h-5" /></button><button type="button" @click.prevent="removeCover" class="bg-red-500 text-white p-2.5 rounded-xl hover:bg-red-600" title="Hapus"><X class="w-5 h-5" /></button></div></div></div>
                </div>
                ...
                <div class="bg-white/80 backdrop-blur-xl rounded-2xl shadow-lg shadow-gray-200/50 border border-gray-100/50 p-6">
                    ...
                    <div v-if="galleryPreviews.length" class="grid grid-cols-3 gap-2 mb-4"><div v-for="(img, i) in galleryPreviews" :key="i" class="relative aspect-square group"><img :src="img" class="w-full h-full object-cover rounded-lg"><div class="absolute inset-0 bg-black/40 rounded-lg flex items-center justify-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity"><button type="button" @click="openGalleryEditor(i)" class="p-1.5 bg-blue-500 text-white rounded-lg hover:bg-blue-600" title="Edit"><Pencil class="w-3 h-3" /></button><button type="button" @click="removeGalleryImage(i)" class="p-1.5 bg-red-500 text-white rounded-lg hover:bg-red-600" title="Hapus"><X class="w-3 h-3" /></button></div></div></div>
