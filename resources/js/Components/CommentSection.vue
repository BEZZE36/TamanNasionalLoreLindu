<script setup>
/**
 * CommentSection.vue - Reusable Comment Component
 * Used for Flora, Fauna, Gallery, and other content types
 * Connects to respective admin comment management
 */
import { ref, computed, onMounted, onBeforeUnmount } from 'vue';
import { usePage, Link } from '@inertiajs/vue3';
import { gsap } from 'gsap';
import { ScrollTrigger } from 'gsap/ScrollTrigger';
import { MessageCircle, Send, Reply, ChevronDown, ChevronUp, Shield, Pin } from 'lucide-vue-next';

gsap.registerPlugin(ScrollTrigger);

const props = defineProps({ 
    itemId: { type: [Number, String], required: true },
    itemType: { type: String, required: true }, // 'flora', 'fauna', 'gallery'  
    comments: { type: Array, default: () => [] },
    colorScheme: { type: String, default: 'emerald' } // 'emerald', 'amber', 'purple'
});
...
const colors = computed(() => {
    ...
    return schemes[props.colorScheme] || schemes.emerald;
});

// API endpoint based on item type
const apiEndpoint = computed(() => `/${props.itemType}/${props.itemId}/comment`);

const formatTimeAgo = (date) => {
    if (!date) return '';
    // Handle both ISO string and "X ago" format
    if (typeof date === 'string' && (date.includes('lalu') || date.includes('ago'))) return date;
    ...
    <div ref="sectionRef" class="bg-white rounded-2xl shadow-lg border border-gray-100 overflow-hidden">
        ...
        <div class="px-4 py-3 border-b border-gray-100 flex items-center justify-between">
            <div class="flex items-center gap-2">
                <div :class="['w-8 h-8 rounded-lg bg-gradient-to-br flex items-center justify-center', colors.gradient]">
                    <MessageCircle class="w-4 h-4 text-white" />
                </div>
                <h2 class="text-sm font-bold text-gray-900">Komentar ({{ totalCommentsCount }})</h2>
            </div>
        </div>

        <!-- Comment Form -->
        <div v-if="user" :class="['p-4 border-b border-gray-100', colors.bg]">
            <div class="flex items-start gap-3">
                <div :class="['w-9 h-9 rounded-full bg-gradient-to-br flex items-center justify-center text-white font-bold text-[11px] flex-shrink-0', colors.gradient]">
                    {{ user?.name?.[0] || 'U' }}
                </div>
                <div class="flex-1">
                    ...
                    <div class="flex justify-end mt-2">
                        <button @click="submitComment()" :disabled="isSubmitting || !commentText.trim()"
                            :class="['inline-flex items-center gap-2 px-4 py-2 bg-gradient-to-r text-white text-[11px] font-semibold rounded-xl hover:shadow-lg transition-all disabled:opacity-50', colors.button, colors.hover]">
        ...
        <div v-else class="p-4 bg-gradient-to-br from-gray-50 to-gray-100 border-b border-gray-100 text-center">
            ...
            <Link href="/login" :class="['text-[11px] font-semibold hover:underline', colors.text]">Login Sekarang</Link>
        </div>
            ...
            <div v-for="comment in localComments" :key="comment.id" 
                ...
                <div class="flex items-start gap-2.5">
                    ...
                    <div class="flex-1 min-w-0">
                        ...
                        <div v-if="replyingTo === comment.id" class="mt-3 flex items-start gap-2">
                            ...
                            <div class="flex flex-col gap-1">
                                <button @click="submitComment(comment.id)" :disabled="isSubmitting || !replyText.trim()"
                                    :class="['px-3 py-2 bg-gradient-to-r text-white text-[10px] font-semibold rounded-lg transition-colors disabled:opacity-50', colors.button]">
                                    <Send class="w-3 h-3" />
                                </button>
                        ...
                        <!-- Replies -->
                        <div v-if="expandedReplies[comment.id] && comment.replies?.length > 0" :class="['mt-3 space-y-2 pl-3 border-l-2', colors.border]">
                                    ...
                                    <div v-if="replyingTo === reply.id" class="mt-2 flex items-start gap-2">
                                        ...
                                        <div class="flex flex-col gap-1">
                                            <button @click="submitComment(reply.id)" :disabled="isSubmitting || !replyText.trim()"
                                                :class="['px-2 py-1.5 bg-gradient-to-r text-white text-[9px] font-semibold rounded-lg transition-colors disabled:opacity-50', colors.button]">
